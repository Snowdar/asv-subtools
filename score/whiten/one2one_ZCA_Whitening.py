...
def transform(train,input,output,matrixPath="",ark_format=False):
	print("Load train data...")
	trainLabel,trainData=ZCA().load_data(train)
	
	print("Load input data...")
	inputLabel,inputData=ZCA().load_data(input)
	...
	print("Transform input data to whitened data...")
	input_data_whitened = list(trf.transform(inputData))
	outputdata(inputLabel,input_data_whitened,output,ark_format)
...
for i in range(1,len(sys.argv)):
    if sys.argv[i].startswith('--'): 
        ...
        n+=1
if len(sys.argv)-n != 3 :
	print('usage: '+sys.argv[0]+' [--ark-format=false|true] [--write-matrix=""] <train-data> <input-data> <output-data>')
	print('e.g.: '+sys.argv[0]+' train.ark input.ark input_whiten.ark')
...
sys.stderr.write(sys.argv[0]+': '+train+' '+input+' '+output+'\n')
print("Run whitenning process ... ")
transform(train,input,output,options["write_matrix"],options["ark_format"])

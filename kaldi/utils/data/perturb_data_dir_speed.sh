...
if [ -f $srcdir/segments ]; then
  ...
  subtools/kaldi/utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/segments | \
    subtools/kaldi/utils/apply_map.pl -f 2 $destdir/reco_map | \
      awk -v factor=$factor \
        '{s=$3/factor; e=$4/factor; if (e > s + 0.01) { printf("%s %s %.2f %.2f\n", $1, $2, $3/factor, $4/factor);} }' >$destdir/segments

  subtools/kaldi/utils/apply_map.pl -f 1 $destdir/reco_map <$srcdir/wav.scp | sed 's/| *$/ |/' | \
  ...
  if [ -f $srcdir/reco2file_and_channel ]; then
    subtools/kaldi/utils/apply_map.pl -f 1 $destdir/reco_map <$srcdir/reco2file_and_channel >$destdir/reco2file_and_channel
  fi

else # no segments->wav indexed by utterance.
  if [ -f $srcdir/wav.scp ]; then
    subtools/kaldi/utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/wav.scp | sed 's/| *$/ |/' | \
     # Handle three cases of rxfilenames appropriately; "input piped command", "file offset" and "filename" 
     awk -v factor=$factor \
       '{wid=$1; $1=""; if ($NF=="|") {print wid $_ " sox -t wav - -t wav - speed " factor " |"}
         else if (match($0, /:[0-9]+$/)) {print wid " wav-copy" $_ " - | sox -t wav - -t wav - speed " factor " |" } 
...
if [ -f $srcdir/utt2lang ]; then
  subtools/kaldi/utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/utt2lang >$destdir/utt2lang
fi
...
cat $srcdir/utt2dur | subtools/kaldi/utils/apply_map.pl -f 1 $destdir/utt_map  | \
...
cat $srcdir/reco2dur | subtools/kaldi/utils/apply_map.pl -f 1 $destdir/reco_map  | \
...
subtools/kaldi/utils/validate_data_dir.sh --no-feats --no-text $destdir

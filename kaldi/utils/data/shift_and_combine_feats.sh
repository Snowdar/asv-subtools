...
if [ $# != 3 ]; then
  ...
  echo "frame-subsampled systems, this script uses subtools/kaldi/utils/data/shift_feats.sh"
  echo "and subtools/kaldi/utils/data/combine_data.sh to shift the features"
...
for frame_shift in `seq $[-(frame_subsampling_factor/2)] $[-(frame_subsampling_factor/2) + frame_subsampling_factor - 1]`; do
  ...
  subtools/kaldi/utils/data/shift_feats.sh $frame_shift $srcdir ${destdir}_fs$frame_shift || exit 1
...
subtools/kaldi/utils/data/combine_data.sh $destdir $srcdir ${tmp_shift_destdirs[@]} || exit 1
rm -r ${tmp_shift_destdirs[@]}

subtools/kaldi/utils/validate_data_dir.sh $destdir

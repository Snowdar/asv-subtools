...
subtools/kaldi/utils/apply_map.pl -f 2 $srcdir/utt2spk < $dir/new2old_utt >$dir/utt2spk
# .. and the new spk2utt file.
subtools/kaldi/utils/utt2spk_to_spk2utt.pl  <$dir/utt2spk >$dir/spk2utt
...
if [ -f $srcdir/segments ]; then
  ...
  subtools/kaldi/utils/apply_map.pl -f 2 $srcdir/segments <$subsegments | \
...
if [ -f $srcdir/utt2uniq ]; then
  subtools/kaldi/utils/apply_map.pl -f 2 $srcdir/utt2uniq <$dir/new2old_utt >$dir/utt2uniq
fi
...
if [ -f $srcdir/feats.scp ]; then
  ...
  if [ ! -s $srcdir/frame_shift ]; then
    frame_shift=$(subtools/kaldi/utils/data/get_frame_shift.sh $srcdir) || exit 1
  else
  ...
  # Lastly, subtools/kaldi/utils/data/normalize_data_range.pl will only do something nontrivial if
  ...
  if [ ! -f $srcdir/utt2num_frames ]; then
    ...
    echo "Increase the number of jobs or write this file while extracting features by passing --write-utt2num-frames true to subtools/kaldi/steps/make_mfcc.sh etc."
  fi
  subtools/kaldi/utils/data/get_utt2num_frames.sh --cmd "$cmd" --nj $nj $srcdir
  awk '{print $1" "$2}' $subsegments | \
    subtools/kaldi/utils/apply_map.pl -f 2 $srcdir/utt2num_frames > \
    $dir/utt2max_frames

  awk -v s=$frame_shift '{print $1, $2, int(($3/s)+0.5), int(($4/s)-0.5);}' <$subsegments| \
    subtools/kaldi/utils/apply_map.pl -f 2 $srcdir/feats.scp | \
    awk '{p=NF-1; for (n=1;n<NF-2;n++) printf("%s ", $n); k=NF-2; l=NF-1; printf("%s[%d:%d]\n", $k, $l, $NF)}' | \
    subtools/kaldi/utils/data/fix_subsegment_feats.pl $dir/utt2max_frames | \
    subtools/kaldi/utils/data/normalize_data_range.pl >$dir/feats.scp || { echo "Failed to create $dir/feats.scp" && exit; }
  ...
  if [ -f $srcdir/vad.scp ]; then
    cat $subsegments | awk -v s=$frame_shift '{print $1, $2, int(($3/s)+0.5), int(($4/s)-0.5);}' | \
      subtools/kaldi/utils/apply_map.pl -f 2 $srcdir/vad.scp | \
      awk '{p=NF-1; for (n=1;n<NF-2;n++) printf("%s ", $n); k=NF-2; l=NF-1; printf("%s[%d:%d]\n", $k, $l, $NF)}' | \
      subtools/kaldi/utils/data/fix_subsegment_feats.pl $dir/utt2max_frames | \
      subtools/kaldi/utils/data/normalize_data_range.pl >$dir/vad.scp
  fi
fi

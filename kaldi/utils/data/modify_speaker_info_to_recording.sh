...
subtools/kaldi/utils/apply_map.pl -f 1 $out_data/old2new.uttmap < $in_data/segments > \
  $out_data/segments || exit 1
awk '{print $1" "$2}' $out_data/segments > $out_data/utt2spk || exit 1
subtools/kaldi/utils/utt2spk_to_spk2utt.pl $out_data/utt2spk > $out_data/spk2utt || exit 1

if [ -f $in_data/text ]; then
  subtools/kaldi/utils/apply_map.pl -f 1 $out_data/old2new.uttmap < $in_data/text > \
    $out_data/text || exit 1
fi

if [ -f $in_data/feats.scp ]; then
  subtools/kaldi/utils/apply_map.pl -f 1 $out_data/old2new.uttmap < $in_data/feats.scp > \
    $out_data/feats.scp || exit 1
fi

subtools/kaldi/utils/fix_data_dir.sh $out_data || exit 1
subtools/kaldi/utils/validate_data_dir.sh --no-text --no-feats $out_data || exit 1

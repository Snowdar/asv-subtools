...
if (-s "$lang/phones/$word_boundary.int") {
  ...
  $text = `. ./subtools/path.sh`;
  if ($text ne "") {
    print "*** This script cannot continue because your subtools/path.sh or bash profile prints something: $text" .
  ...
  foreach $fst ("L.fst", "L_disambig.fst") {
    ...
    $phoneseq = `. ./subtools/path.sh; echo \"$wordseq" | fstcompile | fstcompose $lang/$fst - | fstproject | fstrandgen | fstrmepsilon | fsttopsort | fstprint | awk '{if (NF > 2) {print \$3}}';`;
...
if (-e "$lang/L.fst") {
  ...
  $res = system(". ./subtools/path.sh; $cmd");
  if ($res == 0) {
...
if (-e "$lang/G.fst") {
  ...
  $text = `. ./subtools/path.sh; fstinfo $lang/G.fst`;
  if ($? != 0) {
  ...
  if (-e "$lang/G.fst") {
    system("subtools/kaldi/utils/lang/check_g_properties.pl $lang");
    if ($? != 0) {
      print "--> ERROR: failure running check_g_properties.pl\n";
      $exit = 1;
    } else {
      print("--> subtools/kaldi/utils/lang/check_g_properties.pl succeeded.\n");
    }
  }
}
...
if (!$skip_det_check) {
  if (-e "$lang/G.fst" && -e "$lang/L_disambig.fst") {
    ...
    $output = `. ./subtools/path.sh; fsttablecompose $lang/L_disambig.fst $lang/G.fst | fstdeterminizestar | fstinfo 2>&1 `;

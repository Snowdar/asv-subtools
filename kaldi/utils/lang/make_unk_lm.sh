...
if [ $stage -le 0 ]; then
  if ! subtools/kaldi/utils/validate_dict_dir.pl $dict_dir >&$dir/log/validate_dict_dir.log; then
...
if $use_pocolm; then
  ...
  if [ $stage -le 3 ]; then
    ...
    $cmd $dir/log/arpa2fst.log \
         subtools/kaldi/utils/lang/internal/arpa2fst_constrained.py --verbose=3 \
  ...
  if [ $stage -le 1 ]; then
    ...
    $cmd $dir/log/make_phone_lm.log \
         subtools/kaldi/utils/sym2int.pl $dir/phones.txt $dir/training.txt '|' \
         subtools/kaldi/utils/lang/make_phone_lm.py --verbose=2 \
         --phone-disambig-symbol=$phone_disambig_int \
         --num-extra-ngrams=$num_extra_ngrams \
         --ngram-order=$ngram_order '|' \
         subtools/kaldi/utils/int2sym.pl -f 3-4 $dir/phones.txt '>'$dir/unk_fst_orig.txt
  fi
fi

...
if [ $# -ne 4 ]; then
  echo "Usage: subtools/kaldi/utils/prepare_lang.sh <dict-src-dir> <oov-dict-entry> <tmp-dir> <lang-dir>"
  echo "e.g.: subtools/kaldi/utils/prepare_lang.sh data/local/dict <SPOKEN_NOISE> data/local/lang data/lang"
...
! subtools/kaldi/utils/validate_dict_dir.pl $srcdir && \
...
if ! subtools/kaldi/utils/validate_dict_dir.pl $srcdir >&/dev/null; then
  subtools/kaldi/utils/validate_dict_dir.pl $srcdir  # show the output.
...
if [ ! -z "$extra_word_disambig_syms" ]; then
  if ! subtools/kaldi/utils/lang/validate_disambig_sym_file.pl --allow-numeric "false" $extra_word_disambig_syms; then
...
if $share_silence_phones; then
  ...
  cat $srcdir/silence_phones.txt | awk '{printf("%s ", $0); } END{printf("\n");}' | cat - $srcdir/nonsilence_phones.txt | \
    subtools/kaldi/utils/apply_map.pl $tmpdir/phone_map.txt > $dir/phones/sets.txt
  ...
  cat $srcdir/{,non}silence_phones.txt | subtools/kaldi/utils/apply_map.pl $tmpdir/phone_map.txt > $dir/phones/sets.txt
  cat $dir/phones/sets.txt | awk '{print "shared", "split", $0;}' > $dir/phones/roots.txt
fi

cat $srcdir/silence_phones.txt | subtools/kaldi/utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/silence.txt
cat $srcdir/nonsilence_phones.txt | subtools/kaldi/utils/apply_map.pl $tmpdir/phone_map.txt | \
...
cat $srcdir/extra_questions.txt 2>/dev/null | subtools/kaldi/utils/apply_map.pl $tmpdir/phone_map.txt \
...
if [ ! -z "$unk_fst" ]; then  # if the --unk-fst option was provided...
  if "$silprob"; then
    subtools/kaldi/utils/lang/internal/modify_unk_pron.py $tmpdir/lexiconp_silprob.txt "$oov_word" || exit 1
  else
    subtools/kaldi/utils/lang/internal/modify_unk_pron.py $tmpdir/lexiconp.txt "$oov_word" || exit 1
  fi
...
if "$silprob"; then
  ndisambig=$(subtools/kaldi/utils/add_lex_disambig.pl $unk_opt --pron-probs --sil-probs $tmpdir/lexiconp_silprob.txt $tmpdir/lexiconp_silprob_disambig.txt)
else
  ndisambig=$(subtools/kaldi/utils/add_lex_disambig.pl $unk_opt --pron-probs $tmpdir/lexiconp.txt $tmpdir/lexiconp_disambig.txt)
fi
...
if [ -f $srcdir/nonterminals.txt ]; then
  subtools/kaldi/utils/lang/grammar/augment_phones_txt.py $dir/phones.txt $srcdir/nonterminals.txt $dir/phones.txt
  subtools/kaldi/utils/lang/grammar/augment_words_txt.py $dir/words.txt $srcdir/nonterminals.txt $dir/words.txt
  cp $srcdir/nonterminals.txt $dir/phones/nonterminals.txt
  subtools/kaldi/utils/sym2int.pl $dir/phones.txt <$dir/phones/nonterminals.txt >$dir/phones/nonterminals.int
  ...
  subtools/kaldi/utils/sym2int.pl $dir/phones.txt <$dir/phones/left_context_phones.txt >$dir/phones/left_context_phones.int

  # we need to write subtools/kaldi/utils/lang/make_lexicon_fst_silprob.py before this can work.
...
if $silprob; then
  ...
  # word).  On some setups this helps a bit.  See subtools/kaldi/utils/dict_dir_add_pronprobs.sh
  # and where it's called in the example scripts (run.sh).
  subtools/kaldi/utils/lang/make_lexicon_fst_silprob.py $grammar_opts --sil-phone=$silphone \
...
cat $dir/oov.txt | subtools/kaldi/utils/sym2int.pl $dir/words.txt >$dir/oov.int || exit 1;
...
subtools/kaldi/utils/sym2int.pl $dir/phones.txt <$dir/phones/wdisambig.txt >$dir/phones/wdisambig_phones.int
subtools/kaldi/utils/sym2int.pl $dir/words.txt <$dir/phones/wdisambig.txt >$dir/phones/wdisambig_words.int
...
for f in silence nonsilence optional_silence disambig context_indep; do
  subtools/kaldi/utils/sym2int.pl $dir/phones.txt <$dir/phones/$f.txt >$dir/phones/$f.int
  subtools/kaldi/utils/sym2int.pl $dir/phones.txt <$dir/phones/$f.txt | \
   awk '{printf(":%d", $1);} END{printf "\n"}' | sed s/:// > $dir/phones/$f.csl || exit 1;
done

for x in sets extra_questions; do
  subtools/kaldi/utils/sym2int.pl $dir/phones.txt <$dir/phones/$x.txt > $dir/phones/$x.int || exit 1;
done

subtools/kaldi/utils/sym2int.pl -f 3- $dir/phones.txt <$dir/phones/roots.txt \
   > $dir/phones/roots.int || exit 1;

if [ -f $dir/phones/word_boundary.txt ]; then
  subtools/kaldi/utils/sym2int.pl -f 1 $dir/phones.txt <$dir/phones/word_boundary.txt \
...
# subtools/kaldi/utils/gen_topo.pl.  We do this in the 'chain' recipes.  Of course, the 'topo' file
# should cover all the phones.  Try running subtools/kaldi/utils/validate_lang.pl to check that
# everything is OK after modifying the topo file.
subtools/kaldi/utils/gen_topo.pl $num_nonsil_states $num_sil_states $nonsilphonelist $silphonelist >$dir/topo
...
if $silprob; then
  subtools/kaldi/utils/lang/make_lexicon_fst_silprob.py $grammar_opts \
...
if [ ! -z "$unk_fst" ]; then
  subtools/kaldi/utils/lang/internal/apply_unk_lm.sh $unk_fst $dir || exit 1
...
! subtools/kaldi/utils/validate_lang.pl $dir && echo "$(basename $0): error validating output" &&  exit 1;

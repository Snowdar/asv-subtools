...
# This achieves an equivalent effect of subtools/kaldi/utils/lang/adjust_unk_arpa.pl, which scales
# the LM prob of all ngrams predicting an OOV symbol, while avoiding re-creating the graph.

set -o pipefail

if [ $# != 4 ]; then
   echo "Usage: subtools/kaldi/utils/adjust_unk_graph.sh <oov-dict-entry> <scale> <in-graph-dir> <out-graph-dir>"
   echo "e.g.: subtools/kaldi/utils/adjust_unk_graph.sh \"<unk>\" 0.1 exp/tri1/graph exp/tri1/graph_unk_scale_0.1"
   exit 1;
fi
...
oov_id=`echo $oov_word | subtools/kaldi/utils/sym2int.pl $graphdir_in/words.txt`

...
if [ $# -ne 3 ]; then
  echo "Usage: subtools/kaldi/utils/prepare_subword_text.sh <word-text> <pair_code> <subword-text>"
  echo "e.g.: subtools/kaldi/utils/prepare_subword_text.sh data/train/text data/local/pair_code.txt data/train/text_subword"
...
cut -d ' ' -f2- $word_text | \
  subtools/kaldi/utils/lang/bpe/apply_bpe.py -c $pair_code --separator $separator $glossaires_opt > ${word_text}.sub

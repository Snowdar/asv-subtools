...
if [ -z "$cv_spk_list" ]; then
  ...
  cat $src_data/spk2utt | awk '{ print $1; }' | subtools/kaldi/utils/shuffle_list.pl --srand $seed >$tmp/speakers
...
subtools/kaldi/utils/data/subset_data_dir.sh --spk-list $tmp/speakers_trn $src_data $trn_data
subtools/kaldi/utils/data/subset_data_dir.sh --spk-list $tmp/speakers_cv $src_data $cv_data

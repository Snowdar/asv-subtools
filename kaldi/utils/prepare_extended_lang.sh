...
if [ $# -ne 7 ]; then
  echo "usage: subtools/kaldi/utils/prepare_extended_lang.sh <dict-src-dir> <oov-dict-entry> <extra-lexicon> "
  echo "<phone-symbol-table> <extended-dict-dir> <tmp-dir> <extended-lang-dir>"
  echo "e.g.: subtools/kaldi/utils/prepare_extended_lang.sh data/local/dict '<SPOKEN_NOISE>' lexicon_extra.txt"
  ...
  echo "(see subtools/kaldi/utils/prepare_lang.sh for details). A phone symbol table from a previsouly built "
  echo "lang dir is required, for validating provided lexical entries."
  echo "options: "
  echo "     --prep-lang-opts STRING              # options to pass to subtools/kaldi/utils/prepare_lang.sh"
...
! subtools/kaldi/utils/validate_dict_dir.pl $srcdict && \
...
if [ $stage -le 0 ]; then
  ...
  if ! subtools/kaldi/utils/validate_dict_dir.pl $extdict >&/dev/null; then
    subtools/kaldi/utils/validate_dict_dir.pl $extdict  # show the output.
...
if [ $stage -le 1 ]; then
  ...
  subtools/kaldi/utils/prepare_lang.sh $prep_lang_opts $extdict \

...
def train_new_models(dir, iter, srand, num_jobs,
    ...
    Selected args:
        ...
        use_multitask_egs : True, if different examples used to train multiple
            ...
            subtools/kaldi/steps/nnet3/multilingual/allocate_multilingual_examples.py, those
...
def train_one_iteration(dir, iter, srand, egs_dir,
    ...
    """ Called from subtools/kaldi/steps/nnet3/train_*.py scripts for one iteration of neural
    network training
...
def align(dir, data, lang, run_opts, iter=None,
    ...
    common_lib.execute_command(
        """subtools/kaldi/steps/nnet3/align.sh --nj {num_jobs_align} \
...
def realign(dir, iter, feat_dir, lang, prev_egs_dir, cur_egs_dir,
    ...
    alidir = align(dir, feat_dir, lang, run_opts, iter,
                   online_ivector_dir)
    common_lib.execute_command(
        """subtools/kaldi/steps/nnet3/relabel_egs.sh --cmd "{command}" --iter {iter} \

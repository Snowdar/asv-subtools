...
if (@ARGV == 0) {
  print STDERR "Usage: subtools/kaldi/steps/info/gmm_dir_info.pl [--debug] <gmm-dir1> [<gmm-dir2> ... ]\n" .
               "e.g: subtools/kaldi/steps/info/gmm_dir_info.pl exp/tri3\n" .
...
sub get_num_jobs {
  if (! -d $gmm_dir) {
    print STDERR "subtools/kaldi/steps/info/gmm_dir_info.pl: no such directory $gmm_dir\n";
    exit(1);
  }
  if (!open(F, "<$gmm_dir/num_jobs")) {
    print STDERR "subtools/kaldi/steps/info/gmm_dir_info.pl: no such file $gmm_dir/num_jobs\n";
  }
  my $num_jobs = <F>;
  if (!($num_jobs > 0)) {
    print STDERR "subtools/kaldi/steps/info/gmm_dir_info.pl: bad contents of file $gmm_dir/num_jobs\n";
  }
  close(F);

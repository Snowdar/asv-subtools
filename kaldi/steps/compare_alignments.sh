...
# `set -e` above.
subtools/kaldi/utils/lang/check_phones_compatible.sh $lang1/phones.txt $lang2/phones.txt
...
if [ $stage -le 3 ]; then
  cat $dir/conf.mat | grep -v -F '[' | sed 's/]//' | awk '{n=NF; for (k=1;k<=n;k++) { conf[NR,k] = $k; row_tot[NR] += $k; col_tot[k] += $k; } } END{
   ...
   (
     ...
     grep '^COR' $dir/phone_stats.all | sort -n -k4,4 | awk '{print $2, $3, $4}' | subtools/kaldi/utils/int2sym.pl -f 1 $lang1/phones.txt
   ...
   (
     ...
     grep '^SUB' $dir/phone_stats.all | sort -nr -k2,2 | awk '{print $3,$4,$5,$6,$7}' | subtools/kaldi/utils/int2sym.pl -f 1-2 $lang1/phones.txt
   ) > $dir/phone_subs.txt
fi

if [ $stage -le 4 ]; then
  echo "$0: getting CTMs"
  subtools/kaldi/steps/get_train_ctm.sh --use-segments false --print-silence true --cmd "$cmd" --frame-shift 1.0 $data1 $lang1 $ali_dir1 $dir/ctm1
  subtools/kaldi/steps/get_train_ctm.sh --use-segments false --print-silence true --cmd "$cmd" --frame-shift 1.0 $data2 $lang2 $ali_dir2 $dir/ctm2
fi
...
if [ $stage -le 5 ]; then
  ...
  for n in 1 2; do
    cat $dir/ctm${n}/ctm | subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 5 $lang1/words.txt | \
  ...
  ( echo "# Word stats.  Format:";
    ...
    paste <(awk '{for (n=2;n<NF;n++) print $n;}' <$dir/words_diff.vec) \
      <(awk '{for (n=2;n<NF;n++) print $n;}' <$dir/words_tot.vec) | \
       awk '{ if($2 > 0) print $1*$1/$2, $1/$2, $1, $2, (NR-1)}' | subtools/kaldi/utils/int2sym.pl -f 5 $lang1/words.txt | \
...
if [ $stage -le 8 ]; then
  ...
  for suf in worst mid; do
    for n in 1 2; do
      gunzip -c $dir/phones${n}.gz | copy-int-vector ark:- ark,t:- | subtools/kaldi/utils/filter_scp.pl $dir/utt_ids.$suf  >$dir/temp
      # the next command reorders them, and duplicates the utterance-idwhich we'll later use
      # that to display the word sequence.
      awk '{print $1,$1,$1}' <$dir/utt_ids.$suf | subtools/kaldi/utils/apply_map.pl -f 3 $dir/temp > $dir/phones${n}.$suf
      rm $dir/temp
    done
    ...
    (
      ...
      paste $dir/phones1.$suf $dir/phones2.$suf | perl -ane ' @A = split("\t", $_); @A1 = split(" ", $A[0]); @A2 = split(" ", $A[1]);
            ...
            print "\n" ' | subtools/kaldi/utils/int2sym.pl -f 3- $lang1/phones.txt | sed 's: <eps> :/:g' | \
        subtools/kaldi/utils/apply_map.pl -f 2 $data1/text

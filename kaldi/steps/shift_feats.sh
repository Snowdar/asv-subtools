...
# This script is deprecated. The newer script subtools/kaldi/utils/data/shift_feats.sh
...
if [ $# -ne 4 ]; then
   echo "This script is deprecated. The newer script subtools/kaldi/utils/data/shift_feats.sh"
...
subtools/kaldi/utils/split_data.sh $data_in $nj || exit 1;
...
subtools/kaldi/utils/copy_data_dir.sh --utt-prefix $utt_prefix --spk-prefix $spk_prefix \
  $data_in $data
...
for j in $(seq $nj); do
  ...
  subtools/kaldi/utils/create_data_link.pl $featdir/raw_feats_$name.$j.ark
done

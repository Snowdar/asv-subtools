...
for src in $@; do
  if [[ $src != $first_src ]]; then
    ...
    subtools/kaldi/utils/lang/check_phones_compatible.sh $first_src/phones.txt \
...
do_combine() {
  ...
  $cmd JOB=1:$nj $dest/log/chop_combined_$entities.JOB.log \
    $copy_program \
      "scp:subtools/kaldi/utils/split_scp.pl --utt2spk=$data/utt2spk --one-based -j $nj JOB $temp_dir/$ark.scp |" \

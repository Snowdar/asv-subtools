...
trap "subtools/kaldi/linux/kill_pid_tree.sh --show true $$ && echo -e '\nAll killed'" INT
pids=""
for g in $(seq $nj); do
  $cmd --gpu 1 $logdir/make_mfcc_${name}.$g.log \
    compute-mfcc-feats-cuda $vtln_opts --verbose=2 \
      --config=$mfcc_config scp,p:$logdir/wav_${name}.$g.scp ark:- \| \
    copy-feats "`echo $write_num_frames_opt | sed s/JOB/$g/g`"  --compress=$compress ark:- \
      ark,scp:$mfccdir/raw_mfcc_$name.$g.ark,$mfccdir/raw_mfcc_$name.$g.scp \
      || exit 1 &

  sleep 1
  pids="$pids $!"
done
  trap "subtools/kaldi/linux/kill_pid_tree.sh --show true $pids && echo -e '\nAll killed'" INT
  wait
fi

...
# script in subtools/kaldi/utils/dict_dir_add_pronprobs.sh.
...
if [ $# != 6 ]; then
   echo "Usage: subtools/kaldi/steps/get_lexicon_probs.sh <data-dir> <lang-dir> <src-dir|ali-dir> <old-lexicon> <exp-dir> <new-lexicon>"
   echo "e.g.: subtools/kaldi/steps/get_lexicon_probs.sh data/train data/lang exp/tri5 data/local/lexicon.txt \\"
...
mkdir -p $dir/log
subtools/kaldi/utils/split_data.sh $data $nj # Make sure split data-dir exists.
sdata=$data/split$nj
...
if [ $stage -le 0 ]; then
  ...
  ( ( for n in `seq $nj`; do gunzip -c $srcdir/ali.$n.gz; done ) | \
    linear-to-nbest ark:- "ark:subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt $data/text |" '' '' ark:- | \
    ...
    awk '{ if (NF == 4) { word_phones = sprintf("%s %s", $3, $4); count[word_phones]++; } } 
        END { for(key in count) { print count[key], key; } }' | \
          ...
          subtools/kaldi/utils/int2sym.pl -f 3- $lang/phones.txt  | \
          sed -E 's/_I( |$)/ /g' |  sed -E 's/_E( |$)/ /g' | sed -E 's/_B( |$)/ /g' | sed -E 's/_S( |$)/ /g' | \
          subtools/kaldi/utils/int2sym.pl -f 1 $lang/words.txt > $dir/lexicon_counts.txt
...
if $use_graphs; then
  ...
  if [ $stage -le 0 ]; then
    ...
    tra="ark:subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt $sdata/JOB/text|";   

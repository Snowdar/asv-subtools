...
if [ $stage -le 0 ]; then
  $cmd JOB=1:$nj $dir/log/get_ctm.JOB.log \
    ...
    subtools/kaldi/utils/int2sym.pl -f 5 $lang/words.txt \
    '>' $dir/JOB.ctm
  # Merge and clean,
...
if [ $stage -le 1 ]; then
  subtools/kaldi/steps/conf/convert_ctm_to_tra.py $dir/ctm - | \
  align-text --special-symbol="<eps>" ark:$data/text ark:- ark,t:- | \
  subtools/kaldi/utils/scoring/wer_per_utt_details.pl --special-symbol "<eps>" \
  >$dir/align_text 
  # Append alignment to ctm,
  subtools/kaldi/steps/conf/append_eval_to_ctm.py $dir/align_text $dir/ctm $dir/ctm_aligned
  # Convert words to 'ids',
  cat $dir/ctm_aligned | subtools/kaldi/utils/sym2int.pl -f 5 $lang/words.txt >$dir/ctm_aligned_int
fi

# Prepare word-categories (based on wotd frequencies in 'ctm'),
if [ -z "$category_text" ]; then
  subtools/kaldi/steps/conf/convert_ctm_to_tra.py $dir/ctm - | \
  subtools/kaldi/steps/conf/prepare_word_categories.py --min-count $word_min_count $lang/words.txt - $dir/word_categories
else
  subtools/kaldi/steps/conf/prepare_word_categories.py --min-count $word_min_count $lang/words.txt "$category_text" $dir/word_categories
fi

# Compute lattice-depth,
latdepth=$dir/lattice_frame_depth.ark
if [ $stage -le 2 ]; then
  [ -e $latdepth ] || subtools/kaldi/steps/conf/lattice_depth_per_frame.sh --cmd "$cmd" $latdir $dir
fi

# Create the training data for logistic regression,
if [ $stage -le 3 ]; then
  subtools/kaldi/steps/conf/prepare_calibration_data.py \

...
if [ $# != 6 ]; then
   ...
   echo ""
   echo "Usage: subtools/kaldi/utils/rnnlmrescore.sh <rnn-weight> <old-lang-dir> <rnn-dir> <data-dir> <input-decode-dir> <output-decode-dir>"
   echo "Main options:"
...
if [ $stage -le 5 ]; then
  ...
  for n in `seq $nj`; do
    subtools/kaldi/utils/int2sym.pl -f 2- $oldlang/words.txt < $adir.$n/words > $adir.$n/words_text || exit 1;
    mkdir -p $adir.$n/temp
...
if [ $stage -le 6 ]; then
  echo "$0: invoking subtools/kaldi/utils/rnnlm_compute_scores.sh which calls rnnlm, to get RNN LM scores."
  $cmd JOB=1:$nj $dir/log/rnnlm_compute_scores.JOB.log \
    subtools/kaldi/utils/rnnlm_compute_scores.sh --rnnlm_ver $rnnlm_ver $rnndir $adir.JOB/temp $adir.JOB/words_text $adir.JOB/lmwt.rnn \
    || exit 1;
fi
if [ $stage -le 7 ]; then

...
# Warning, this script is deprecated, please use subtools/kaldi/utils/data/modify_speaker_info.sh

# This script is as subtools/kaldi/utils/copy_data_dir.sh in that it copies a data-dir,
...
echo "$0: this script is deprecated, please use subtools/kaldi/utils/data/modify_speaker_info.sh."
...
if [ "$utts_per_spk_max" != -1 ]; then
  ...
  if [ -f $srcdir/cmvn.scp ]; then
    ...
    subtools/kaldi/utils/apply_map.pl -f 2 $srcdir/cmvn.scp <$srcdir/utt2spk | \
      subtools/kaldi/utils/apply_map.pl -f 1 $destdir/utt2spk | sort | uniq > $destdir/cmvn.scp
    echo "$0: mapping cmvn.scp, but you may want to recompute it if it's needed,"
    echo " as it would probably change."
  fi
  if [ -f $srcdir/spk2gender ]; then
    subtools/kaldi/utils/apply_map.pl -f 2 $srcdir/spk2gender <$srcdir/utt2spk | \
      subtools/kaldi/utils/apply_map.pl -f 1 $destdir/utt2spk | sort | uniq >$destdir/spk2gender
  fi
else
...
subtools/kaldi/utils/validate_data_dir.sh $opts $destdir

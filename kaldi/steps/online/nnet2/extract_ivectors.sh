...
sdata=$data/split$nj;
subtools/kaldi/utils/split_data.sh $data $nj || exit 1;
...
"> $dir/spk2utt || exit 1;
    ...
    for j in $(seq $nj); do
      ...
      subtools/kaldi/utils/filter_scp.pl -f 2 $sdata/$j/utt2spk <$dir/spk2utt >$dir/split$nj/$j/spk2utt || exit 1;
      subtools/kaldi/utils/spk2utt_to_utt2spk.pl <$dir/split$nj/$j/spk2utt >$dir/split$nj/$j/utt2spk || exit 1;
    done
  fi
...
if [ $stage -le 3 ]; then
  for j in $(seq $nj); do
    subtools/kaldi/utils/apply_map.pl -f 2 $dir/ivectors_spk.$j.ark <$this_sdata/$j/utt2spk >$dir/ivectors_utt.$j.ark || exit 1;
  done
fi
...
absdir=$(subtools/kaldi/utils/make_absolute.sh $dir)
...
subtools/kaldi/steps/nnet2/get_ivector_id.sh $srcdir > $dir/final.ie.id || exit 1

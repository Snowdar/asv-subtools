...
# See also subtools/kaldi/steps/get_ctm.sh
...
if [ $# -ne 3 ]; then
  ...
  echo "See also: subtools/kaldi/steps/get_ctm.sh, subtools/kaldi/steps/get_ctm_conf_fast.sh"
  exit 1;
fi
...
if [ $stage -le 0 ]; then
  if [ -f $data/segments ] && $use_segments; then
    ...
    [ ! -f $f ] && echo "$0: expecting file $f to exist" && exit 1;
    filter_cmd="subtools/kaldi/utils/convert_ctm.pl $data/segments $data/reco2file_and_channel"
  else
    filter_cmd=cat
  fi

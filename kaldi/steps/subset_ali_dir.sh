...
if [ $# -ne 4 ]; then
  ...
  Options: 
      --cmd (subtools/kaldi/utils/run.pl|subtools/kaldi/utils/queue.pl <queue opts>)  # how to run jobs.
EOF
  exit 1
fi
...
subtools/kaldi/utils/split_data.sh $subset_data $nj
$cmd JOB=1:$nj $dir/log/filter_alignments.JOB.log \
  copy-int-vector \
  "scp:subtools/kaldi/utils/filter_scp.pl $subset_data/split${nj}/JOB/utt2spk $dir/ali_tmp.scp |" \

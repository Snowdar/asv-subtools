...
# subtools/kaldi/utils/shuffle_list.pl is deterministic, unlike sort -R.
cat $data/utt2spk | awk '{print $1}' | subtools/kaldi/utils/shuffle_list.pl | \
...
if [ $stage -le -6 ]; then
  ...
  if [ -f $data/segments ]; then
    ...
    subset_feats="subtools/kaldi/utils/filter_scp.pl $dir/utt_subset $data/segments | extract-segments scp:$data/wav.scp - ark:- "
  else
    echo "$0 [info]: no segments file exists: using wav.scp directly."
    subset_feats="subtools/kaldi/utils/filter_scp.pl $dir/utt_subset $data/wav.scp | wav-copy scp:- ark:- "
  fi
...
if ! subtools/kaldi/utils/filter_scp.pl $dir/utt_subset $data/feats.scp | \
...
if [ $stage -le 0 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/compile_graphs.JOB.log \
    compile-train-graphs --read-disambig-syms=$lang/phones/disambig.int $dir/tree $dir/1.mdl  $lang/L.fst  \
     "ark:subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt < $data/split$nj/JOB/text |" \
...
if true; then # Diagnostics
  ...
  if [ -f $data/spk2gender ]; then 
    ...
    for g in m f; do # means: for gender in male female
      cat $dir/final.warp | \
        subtools/kaldi/utils/filter_scp.pl <(grep -w $g $data/spk2gender | awk '{print $1}') > $dir/final.warp.$g
...
subtools/kaldi/utils/summarize_warnings.pl  $dir/log

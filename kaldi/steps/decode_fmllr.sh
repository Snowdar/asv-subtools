...
if [ $# != 3 ]; then
   ...
   echo "Usage: subtools/kaldi/steps/decode_fmllr.sh [options] <graph-dir> <data-dir> <decode-dir>"
   echo " e.g.: subtools/kaldi/steps/decode_fmllr.sh exp/tri2b/graph_tgpr data/test_dev93 exp/tri2b/decode_dev93_tgpr"
...
if [ -z "$si_dir" ]; then # we need to do the speaker-independent decoding pass.
  ...
  if [ $stage -le 0 ]; then
    ...
    subtools/kaldi/steps/decode.sh --scoring-opts "$scoring_opts" \
...
if [ $stage -le 5 ]; then
  subtools/kaldi/steps/diagnostic/analyze_lats.sh --cmd "$cmd" $graphdir $dir
fi

...
# This script post-processes the output of subtools/kaldi/steps/segmentation/decode_sad.sh,
...
if [ $# -ne 3 ]; then
  echo "This script post-processes the output of subtools/kaldi/steps/segmentation/decode_sad.sh, "
...
subtools/kaldi/utils/split_data.sh $data_dir $nj

subtools/kaldi/utils/data/get_utt2dur.sh $data_dir

if [ $stage -le 0 ]; then
  $cmd JOB=1:$nj $dir/log/segmentation.JOB.log \
    copy-int-vector "ark:gunzip -c $vad_dir/ali.JOB.gz |" ark,t:- \| \
    subtools/kaldi/steps/segmentation/internal/sad_to_segments.py \

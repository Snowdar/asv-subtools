...
if $convert_data_dir_to_whole; then
  ...
  if [ $stage -le 0 ]; then
    ...
    subtools/kaldi/utils/data/convert_data_dir_to_whole.sh $src_data_dir ${test_data_dir}
  fi
else
  test_data_dir=data/${data_id}${feat_affix}_hires
  if [ $stage -le 0 ]; then
    rm -r ${test_data_dir} || true
    subtools/kaldi/utils/copy_data_dir.sh $src_data_dir $test_data_dir
  fi
fi
...
if [ $stage -le 1 ]; then
  subtools/kaldi/utils/fix_data_dir.sh $test_data_dir
  subtools/kaldi/steps/make_mfcc.sh --mfcc-config $mfcc_config --nj $nj --cmd "$cmd" --write-utt2num-frames true \
    ${test_data_dir} exp/make_hires$feat_affix/${data_id} $mfcc_dir
  subtools/kaldi/steps/compute_cmvn_stats.sh ${test_data_dir} exp/make_hires$feat_affix/${data_id} $mfcc_dir
  subtools/kaldi/utils/fix_data_dir.sh ${test_data_dir}
fi
...
if [ $stage -le 4 ]; then
  ...
  subtools/kaldi/steps/nnet3/compute_output.sh --nj $nj --cmd "$cmd" \
    --iter ${iter} \
...
subtools/kaldi/utils/data/get_utt2dur.sh --nj $nj --cmd "$cmd" $test_data_dir || exit 1
frame_shift=$(subtools/kaldi/utils/data/get_frame_shift.sh $test_data_dir) || exit 1
...
EOF
  ...
  $cmd $graph_dir/log/make_graph.log \
    subtools/kaldi/steps/segmentation/internal/prepare_sad_graph.py $graph_opts \
...
mkdir -p $seg_dir
if [ $stage -le 6 ]; then
  subtools/kaldi/steps/segmentation/internal/get_transform_probs_mat.py \
    --priors="$post_vec" $transform_probs_opts > $seg_dir/transform_probs.mat

  subtools/kaldi/steps/segmentation/decode_sad.sh --acwt $acwt --cmd "$cmd" \
    --nj $nj \
...
if [ $stage -le 7 ]; then
  subtools/kaldi/steps/segmentation/post_process_sad_to_segments.sh \
...
if [ $stage -le 8 ]; then
  subtools/kaldi/utils/data/subsegment_data_dir.sh ${test_data_dir} ${seg_dir}/segments \
    ${data_dir}_seg
  cp $src_data_dir/wav.scp ${data_dir}_seg
  cp $src_data_dir/{stm,reco2file_and_channel,glm} ${data_dir}_seg/ || true
  subtools/kaldi/utils/fix_data_dir.sh ${data_dir}_seg
fi

...
# See subtools/kaldi/steps/segmentation/lats_to_targets.sh for details about the 
...
# removed in the script subtools/kaldi/steps/segmentation/merge_targets_dirs.sh.
...
[ -f ./subtools/path.sh ] && . ./subtools/path.sh 
...
subtools/kaldi/utils/validate_data_dir.sh $in_data_dir || exit 1
subtools/kaldi/utils/validate_data_dir.sh --no-text $in_whole_data_dir || exit 1

if ! cat $garbage_phones_list $silence_phones_list | \
  subtools/kaldi/steps/segmentation/internal/verify_phones_list.py $lang/phones.txt; then
...
if [ $stage -le 0 ]; then
  ...
  subtools/kaldi/utils/data/modify_speaker_info_to_recording.sh \
    $in_data_dir $dir/$data_id || exit 1
  subtools/kaldi/utils/validate_data_dir.sh --no-feats $dir/$data_id || exit 1
fi 
...
###############################################################################
if [ $stage -le 1 ]; then
  subtools/kaldi/utils/data/subsegment_data_dir.sh $in_whole_data_dir ${data_dir}/segments ${data_dir}/tmp
  cp $data_dir/tmp/feats.scp $data_dir

  subtools/kaldi/steps/compute_cmvn_stats.sh $data_dir || exit 1
fi

if [ $stage -le 2 ]; then
  subtools/kaldi/utils/copy_data_dir.sh $in_whole_data_dir $dir/$whole_data_id

  subtools/kaldi/utils/fix_data_dir.sh $dir/$whole_data_id
...
if [ $stage -le 2 ]; then
  subtools/kaldi/steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" \
...
if [ $stage -le 3 ]; then
  subtools/kaldi/utils/data/get_segments_for_data.sh ${whole_data_dir} > \
  ...
  subtools/kaldi/utils/data/subsegment_data_dir.sh $whole_data_dir \
...
if [ -z "$graph_dir" ]; then
  ...
  if [ $stage -le 4 ]; then
    if [ ! -f $graph_dir/HCLG.fst ]; then
      ...
      subtools/kaldi/utils/mkgraph.sh $dir/lang_test $model_dir $graph_dir || exit 1
    fi
  fi
fi
...
if [ $stage -le 5 ]; then 
  ...
  subtools/kaldi/steps/decode.sh --cmd "$decode_cmd --mem 2G" --nj $nj \
...
###############################################################################
if [ $stage -le 6 ]; then
  subtools/kaldi/steps/segmentation/lats_to_targets.sh --cmd "$train_cmd" \
...
###############################################################################
if [ $stage -le 8 ]; then
  subtools/kaldi/steps/segmentation/convert_targets_dir_to_whole_recording.sh --cmd "$train_cmd" --nj $reco_nj \
    $data_dir $whole_data_dir \
    $dir/${ali_model_id}_${data_id}_sup_targets \
    $dir/${ali_model_id}_${whole_data_id}_sup_targets
  
  subtools/kaldi/steps/segmentation/resample_targets_dir.sh --cmd "$train_cmd" --nj $reco_nj 3 \
    $whole_data_dir \
    $dir/${ali_model_id}_${whole_data_id}_sup_targets \
...
if [ $stage -le 10 ]; then
  ...
  subtools/kaldi/steps/segmentation/get_targets_for_out_of_segments.sh --cmd "$train_cmd" \
...
###############################################################################
if [ $stage -le 11 ]; then
  subtools/kaldi/steps/segmentation/merge_targets_dirs.sh --cmd "$train_cmd" --nj $reco_nj \

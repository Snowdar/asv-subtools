...
# single targets matrices. See subtools/kaldi/steps/segmentation/lats_to_targets.sh for 
...
if [ $# -lt 3 ]; then
  ...
  Usage: subtools/kaldi/steps/segmentation/merge_targets_dirs.py <data> <targets-1> <targets-2> ... <merged-targets>
  e.g.: subtools/kaldi/steps/segmentation/merge_targets_dirs.py --weights 1.0,0.5 \
      data/train_whole \
...
for t in ${targets_dirs[@]}; do
  ...
  subtools/kaldi/utils/filter_scps.pl JOB=1:$nj $sdata/JOB/utt2spk \
...
$cmd JOB=1:$nj $dir/log/merge_targets.JOB.log \
  ...
  subtools/kaldi/steps/segmentation/internal/merge_targets.py --weights="$weights" \

...
# This is similar to the script subtools/kaldi/steps/segmentation/lats_to_targets.sh which 
...
if [ $# -ne 4 ]; then
  ...
  Usage: subtools/kaldi/steps/segmentation/ali_to_targets.sh <data-dir> <lang> <ali-dir> <targets-dir>"
  e.g.: subtools/kaldi/steps/segmentation/ali_to_targets.sh \
...
if [ -z "$garbage_phones" ]; then
  oov_phone=$(subtools/kaldi/steps/segmentation/internal/get_oov_phone.py $lang) || exit 1
  echo $oov_phone | subtools/kaldi/utils/int2sym.pl $lang/phones.txt > $dir/garbage_phones.txt || exit 1
else 
...
if [ -z "$silence_phones" ]; then
  cat $lang/silence_phones.txt | \
    subtools/kaldi/utils/filter_scp.pl --exclude $dir/garbage_phones.txt > \
...
$cmd JOB=1:$nj $dir/log/get_arc_info.JOB.log \
  ...
  subtools/kaldi/utils/int2sym.pl -f 5 $lang/phones.txt \| \
...
$cmd JOB=1:$nj $dir/log/get_targets.JOB.log \
  subtools/kaldi/steps/segmentation/internal/arc_info_to_targets.py \
...
subtools/kaldi/steps/segmentation/validate_targets_dir.sh $dir $data || exit 1

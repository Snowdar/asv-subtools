...
if [ $# -ne 2 ]; then
  ...
  Usage: subtools/kaldi/steps/segmentation/validate_targets_dir.sh <targets-dir> <data-dir>
  e.g.: subtools/kaldi/steps/segmentation/validate_targets_dir.sh \
...
subtools/kaldi/utils/data/validate_data_dir.sh --no-text --no-wav --no-spk-sort \
  $data || exit 1
...
subtools/kaldi/utils/filter_scp.pl $tmpdir/len.targets $data/feats.scp | sort -k1,1 | feat-to-len scp:- ark,t:$tmpdir/len.feats || exit 1
...
subtools/kaldi/utils/filter_scp.pl $tmpdir/len.feats $tmpdir/len.targets | \

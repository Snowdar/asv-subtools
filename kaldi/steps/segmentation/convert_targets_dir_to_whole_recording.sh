...
# Note that the script subtools/kaldi/steps/segmentation/get_targets_for_out_of_segments.sh 
...
# combined using the weights in subtools/kaldi/steps/segmentation/merge_targets_dirs.sh
...
if [ $# -ne 4 ]; then
  ...
  Usage: subtools/kaldi/steps/segmentation/convert_targets_to_whole_recording.sh <data-dir> <whole-data-dir> <targets-dir> <whole-targets-dir>
   e.g.: subtools/kaldi/steps/segmentation/convert_targets_to_whole_recording.sh \
...
if [ ! -f $data/segments ]; then
  ...
  subtools/kaldi/utils/filter_scp.pl $data/utt2spk $dir/recos > $dir/recos.data
...
subtools/kaldi/utils/split_scp.pl $whole_data/wav.scp $split_scps

subtools/kaldi/utils/data/get_reco2utt_for_data.sh $data > $dir/reco2utt

mkdir -p $dir/split${nj}reco
subtools/kaldi/utils/filter_scps.pl JOB=1:$nj $dir/split${nj}reco/wav.JOB.scp $dir/reco2utt \
  $dir/split${nj}reco/reco2utt.JOB || exit 1
subtools/kaldi/utils/filter_scps.pl -f 2 JOB=1:$nj $dir/split${nj}reco/wav.JOB.scp $data/segments \
    $dir/split${nj}reco/segments.JOB || exit 1
subtools/kaldi/utils/filter_scps.pl JOB=1:$nj $dir/split${nj}reco/segments.JOB $targets_dir/targets.scp \
    $dir/split${nj}reco/targets.JOB.scp || exit 1

# make $dir an absolute pathname.
dir=`perl -e '($dir,$pwd)= @ARGV; if($dir!~m:^/:) { $dir = "$pwd/$dir"; } print $dir; ' $dir ${PWD}`

subtools/kaldi/utils/data/get_utt2num_frames.sh --cmd "$cmd" --nj $nj $whole_data
cp $whole_data/utt2num_frames $dir/reco2num_frames

$cmd JOB=1:$nj $dir/log/merge_targets_to_reco.JOB.log \
  subtools/kaldi/steps/segmentation/internal/merge_segment_targets_to_recording.py \
...
subtools/kaldi/steps/segmentation/validate_targets_dir.sh $dir $whole_data || exit 1

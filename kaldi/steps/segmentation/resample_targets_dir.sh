...
# This script is a wrapper to subtools/kaldi/steps/segmentation/interal/resample_targets.py,
# which works very similar to the binary subsample-feats. See that script
# for details about how the resampling is done.

# See the script subtools/kaldi/steps/segmentation/lats_to_targets.sh for details about 
...
if [ $# -ne 4 ]; then
  ...
  Usage: subtools/kaldi/steps/segmentation/resample_targets.sh <subsampling-factor> <data-dir> <targets-dir> <resampled-targets-dir>
   e.g.: subtools/kaldi/steps/segmentation/resample_targets.sh 3 \
    data/train_whole \
...
subtools/kaldi/steps/segmentation/validate_targets_dir.sh $targets_dir $data || exit 1
...
subtools/kaldi/utils/split_scp.pl $targets_dir/targets.scp $split_scps
...
if [ $subsampling_factor -eq 1 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/resample_targets.JOB.log \
    ...
    subtools/kaldi/steps/segmentation/internal/resample_targets.py \
...
subtools/kaldi/steps/segmentation/validate_targets_dir.sh $targets_dir $data

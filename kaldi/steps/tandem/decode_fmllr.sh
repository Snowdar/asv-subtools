...
if [ $# != 4 ]; then
   echo "Usage: subtools/kaldi/steps/decode_fmllr.sh [options] <graph-dir> <data1-dir> <data2-dir> <decode-dir>"
   echo " e.g.: subtools/kaldi/steps/decode_fmllr.sh exp/tri2b/graph {mfcc,bottleneck}/data/test_dev93 exp/tri2b/decode_dev93"
...
if [ -z "$si_dir" ]; then # we need to do the speaker-independent decoding pass.
  ...
  if [ $stage -le 0 ]; then
    subtools/kaldi/steps/tandem/decode_si.sh --acwt $acwt --nj $nj --cmd "$cmd" --beam $first_beam --model $alignment_model --max-active $first_max_active $graphdir $data1 $data2 $si_dir || exit 1;
  fi
fi
##

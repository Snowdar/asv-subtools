...
if [ $# != 5 ]; then
   echo "Usage: subtools/kaldi/steps/tandem/make_denlats_sgmm2.sh [options] <data1-dir> <data2-dir> <lang-dir> <src-dir|srcdir> <exp-dir>"
   echo "  e.g.: subtools/kaldi/steps/tandem/make_denlats_sgmm2.sh {mfcc,bottleneck}/data1/train data1/lang exp/sgmm4a_ali exp/sgmm4a_denlats"
...
cat $data1/text | subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt | \
  awk '{for(n=2;n<=NF;n++){ printf("%s ", $n); } printf("\n"); }' | \
  subtools/kaldi/utils/make_unigram_grammar.pl | fstcompile > $dir/lang/G.fst \
   || exit 1;
...
if [ -s $dir/dengraph/HCLG.fst ]; then
  ...
  subtools/kaldi/utils/mkgraph.sh $dir/lang $srcdir $dir/dengraph || exit 1;
fi

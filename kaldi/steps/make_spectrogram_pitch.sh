...
# Note: This file is based on make_spectrogram.sh and make_pitch_kaldi.sh
...
if [ $# -ge 3 ]; then
  spectrogram_pitch_dir=$3
else
  spectrogram_pitch_dir=$data/data
fi


# make $spectrogram_pitch_dir an absolute pathname.
spectrogram_pitch_dir=`perl -e '($dir,$pwd)= @ARGV; if($dir!~m:^/:) { $dir = "$pwd/$dir"; } print $dir; ' $spectrogram_pitch_dir ${PWD}`

# use "name" as part of name of the archive.
name=`basename $data`

mkdir -p $spectrogram_pitch_dir || exit 1;
...
required="$scp $spectrogram_config $pitch_config"

for f in $required; do
  if [ ! -f $f ]; then
    echo "make_spectrogram_pitch.sh: no such file $f"
    exit 1;
  fi
done
...
for n in $(seq $nj); do
  # the next command does nothing unless $spectrogram_pitch_dir/storage/ exists, see
  # subtools/kaldi/utils/create_data_link.pl for more info.
  subtools/kaldi/utils/create_data_link.pl $spectrogram_pitch_dir/raw_spectrogram_pitch_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  spectrogram_feats="ark:extract-segments scp,p:$scp $logdir/segments.JOB ark:- | compute-spectrogram-feats --verbose=2 --config=$spectrogram_config ark:- ark:- |"
  ...
  $cmd JOB=1:$nj $logdir/make_spectrogram_pitch_${name}.JOB.log \
    paste-feats --length-tolerance=$paste_length_tolerance "$spectrogram_feats" "$pitch_feats" ark:- \| \
    copy-feats --compress=$compress $write_num_frames_opt ark:- \
      ark,scp:$spectrogram_pitch_dir/raw_spectrogram_pitch_$name.JOB.ark,$spectrogram_pitch_dir/raw_spectrogram_pitch_$name.JOB.scp \
     || exit 1;
  ...
  spectrogram_feats="ark:compute-spectrogram-feats --verbose=2 --config=$spectrogram_config scp,p:$logdir/wav.JOB.scp ark:- |"
...
if [ -f $logdir/.error.$name ]; then
  echo "Error producing spectrogram & pitch features for $name:"
  tail $logdir/make_spectrogram_pitch_${name}.1.log
  exit 1;
fi

# concatenate the .scp files together.
for n in $(seq $nj); do
  cat $spectrogram_pitch_dir/raw_spectrogram_pitch_$name.$n.scp || exit 1;
done > $data/feats.scp

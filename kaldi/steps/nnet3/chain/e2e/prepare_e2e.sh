...
if [ $# != 3 ]; then
  echo "Usage: subtools/kaldi/steps/prepare_e2e.sh [options] <data-dir> <lang-dir> <exp-dir>"
  echo " e.g.: subtools/kaldi/steps/prepare_e2e.sh data/train data/lang_chain exp/chain/e2e_tree"
...
if $tie && [[ "$type" = "biphone" ]]; then
  cat $data/text | subtools/kaldi/steps/chain/e2e/text_to_phones.py --edge-silprob 0 \
                                                     --between-silprob 0 \
                                                     $lang | \
    cut -d' ' -f 2- | subtools/kaldi/utils/sym2int.pl $lang/phones.txt | \
    subtools/kaldi/steps/chain/e2e/compute_biphone_stats.py $lang >$dir/phone-stats.txt

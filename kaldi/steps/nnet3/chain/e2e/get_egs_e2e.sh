...
online_cmvn=false # Set to 'true' to replace 'apply-cmvn' by 'apply-cmvn-online' in the nnet3 input.
                  ...
                  #       subtools/kaldi/steps/online/nnet2/train_ivector_extractor.sh by --online-cmvn-iextractor
...
cat $data/utt2len | \
  awk '{print $1}' | \
  subtools/kaldi/utils/shuffle_list.pl 2>/dev/null | head -$num_utts_subset > $dir/valid_uttlist
...
cat $data/utt2len | \
  awk '{print $1}' | \
   subtools/kaldi/utils/filter_scp.pl --exclude $dir/valid_uttlist | \
   subtools/kaldi/utils/shuffle_list.pl 2>/dev/null | head -$num_utts_subset > $dir/train_subset_uttlist
...
if ! $online_cmvn; then
  ...
  feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl --exclude $dir/valid_uttlist $sdata/JOB/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- |"
  ...
  feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl --exclude $dir/valid_uttlist $sdata/JOB/feats.scp | apply-cmvn-online $cmvn_opts --spk2utt=ark:$sdata/JOB/spk2utt $dir/global_cmvn.stats scp:- ark:- |"
  valid_feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/valid_uttlist $data/feats.scp | apply-cmvn-online $cmvn_opts --spk2utt=ark:$data/spk2utt $dir/global_cmvn.stats scp:- ark:- |"
  train_subset_feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/train_subset_uttlist $data/feats.scp | apply-cmvn-online $cmvn_opts --spk2utt=ark:$data/spk2utt $dir/global_cmvn.stats scp:- ark:- |"
fi
...
if [ ! -z "$online_ivector_dir" ]; then
  ...
  subtools/kaldi/steps/nnet2/get_ivector_id.sh $online_ivector_dir > $dir/info/final.ie.id || exit 1
...
if [ -e $dir/storage ]; then
  ...
  subtools/kaldi/utils/create_data_link.pl $(for x in $(seq $num_archives); do echo $dir/cegs.$x.ark; done)
  for x in $(seq $num_archives_intermediate); do
    subtools/kaldi/utils/create_data_link.pl $(for y in $(seq $nj); do echo $dir/cegs_orig.$y.$x.ark; done)
  done
fi
...
if [ $stage -le 3 ]; then
  ...
  $cmd $dir/log/create_valid_subset.log \
    subtools/kaldi/utils/filter_scp.pl $dir/valid_uttlist $fstdir/fst_special.scp \| \
    fstcopy scp:- ark:- \| \
  ...
  $cmd $dir/log/create_train_subset.log \
    subtools/kaldi/utils/filter_scp.pl $dir/train_subset_uttlist $fstdir/fst_special.scp \| \
    fstcopy scp:- ark:- \| \
    nnet3-chain-e2e-get-egs $ivector_opts --srand=$srand \
      $egs_opts $norm_opt \
...
if [ $stage -le 4 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/get_egs.JOB.log \
    subtools/kaldi/utils/filter_scp.pl $sdata/JOB/utt2spk $fstdir/fst.scp \| \
    fstcopy scp:- ark:- \| \

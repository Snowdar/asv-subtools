...
# Copy phones.txt from ali-dir to dir. Later, subtools/kaldi/steps/nnet3/decode.sh will
...
if [ $stage -le -5 ]; then
  ...
  python subtools/kaldi/steps/nnet3/make_tdnn_configs.py  \
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/kaldi/steps/nnet3/get_egs.sh $egs_opts "${extra_opts[@]}" \
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      subtools/kaldi/steps/nnet3/align.sh --nj $num_jobs_align --cmd "$align_cmd" --use-gpu $align_use_gpu \
        --transform-dir "$transform_dir" --online-ivector-dir "$online_ivector_dir" \
        --iter $x $data $lang $dir $dir/ali_$time || exit 1

      subtools/kaldi/steps/nnet3/relabel_egs.sh --cmd "$cmd" --iter $x $dir/ali_$time \
        $prev_egs_dir $cur_egs_dir || exit 1

      if $cleanup && [[ $prev_egs_dir =~ $dir/egs* ]]; then
        subtools/kaldi/steps/nnet3/remove_egs.sh $prev_egs_dir
      fi
    fi
...
subtools/kaldi/steps/info/nnet3_dir_info.pl $dir

...
word_determinize=false  # If set to true, then output lattice does not retain
                        ...
                        # Setting to true is the default in subtools/kaldi/steps/nnet3/decode.sh.
...
if [ $# -ne 3 ]; then
  ...
  echo "e.g.:   subtools/kaldi/steps/nnet3/decode.sh --nj 8 \\"
...
subtools/kaldi/utils/lang/check_phones_compatible.sh {$srcdir,$graphdir}/phones.txt || exit 1
...
if [ $stage -le 2 ]; then
  if ! $skip_diagnostics ; then
    ...
    subtools/kaldi/steps/diagnostic/analyze_lats.sh --cmd "$cmd" $iter_opt $graphdir $dir
  fi
fi

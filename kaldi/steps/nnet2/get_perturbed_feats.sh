...
for pair in $pairs; do
  ...
  subtools/kaldi/utils/copy_data_dir.sh --spk-prefix ${pair}- --utt-prefix ${pair}- $inputdata $feature_data
  subtools/kaldi/steps/make_${feature_type}.sh --${feature_type}-config $conf --nj "$nj" --cmd "$cmd" $feature_data $this_dir $featdir

  subtools/kaldi/steps/compute_cmvn_stats.sh $feature_data $this_dir $featdir
done

subtools/kaldi/utils/combine_data.sh $data $all_feature_dirs
...
# script subtools/kaldi/steps/nnet2/get_egs.sh to hold out data in a more proper way.
cat $data/utt2spk | \

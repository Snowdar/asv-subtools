...
if [ $# != 3 ]; then
  echo "Usage: subtools/kaldi/steps/nnet2/relabel_egs.sh [opts] <ali-dir> <egs-in-dir> <egs-out-dir>"
  echo "  e.g: subtools/kaldi/steps/nnet2/relabel_egs.sh exp/tri6_nnet/ali_1.5 exp/tri6_nnet/egs exp/tri6_nnet/egs_1.5"
  echo ""
...
if [ $stage -le 0 ]; then
  ...
  for x in `seq 1 $num_jobs_nnet`; do
    for y in `seq 0 $[$iters_per_epoch-1]`; do
      subtools/kaldi/utils/create_data_link.pl $dir/egs.$x.$y.ark
      if [ $x -eq 1 ]; then
...
if [ $stage -le 1 ]; then
  ...
  for x in combine.egs valid_diagnostic.egs train_diagnostic.egs $extra_egs; do
    subtools/kaldi/utils/create_data_link.pl $dir/$x

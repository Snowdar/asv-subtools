...
if [ $stage -le -4 ]; then
  ...
  subtools/kaldi/steps/nnet2/get_lda.sh $lda_opts "${extra_opts[@]}" --cmd "$cmd" $data $lang $alidir $dir || exit 1;
fi
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/kaldi/steps/nnet2/get_egs2.sh $egs_opts "${extra_opts[@]}"  --io-opts "$io_opts" \

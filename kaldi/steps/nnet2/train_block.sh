...
if [ $stage -le -4 ]; then
  ...
  subtools/kaldi/steps/nnet2/get_lda_block.sh --block-size $block_size --block-shift $block_shift \
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/kaldi/steps/nnet2/get_egs.sh --io-opts "$io_opts" --samples-per-iter $samples_per_iter \
...
if $cleanup; then
  ...
  if [ $egs_dir == "$dir/egs" ]; then
    subtools/kaldi/steps/nnet2/remove_egs.sh $dir/egs
  fi

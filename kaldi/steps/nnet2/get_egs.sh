...
if [ $# != 4 ]; then
  echo "Usage: subtools/kaldi/steps/nnet2/get_egs.sh [opts] <data> <lang> <ali-dir> <exp-dir>"
  echo " e.g.: subtools/kaldi/steps/nnet2/get_egs.sh data/train data/lang exp/tri3_ali exp/tri4_nnet"
  echo ""
...
sdata=$data/split$nj
subtools/kaldi/utils/split_data.sh $data $nj

mkdir -p $dir/log
cp $alidir/tree $dir

subtools/kaldi/utils/lang/check_phones_compatible.sh $lang/phones.txt $alidir/phones.txt || exit 1;
cp $lang/phones.txt $dir || exit 1;

# Get list of validation utterances.
awk '{print $1}' $data/utt2spk | subtools/kaldi/utils/shuffle_list.pl | head -$num_utts_subset \
...
if [ -f $data/utt2uniq ]; then
  ...
  subtools/kaldi/utils/utt2spk_to_spk2utt.pl $data/utt2uniq > $dir/uniq2utt
  cat $dir/valid_uttlist.tmp | subtools/kaldi/utils/apply_map.pl $data/utt2uniq | \
    sort | uniq | subtools/kaldi/utils/apply_map.pl $dir/uniq2utt | \
    awk '{for(n=1;n<=NF;n++) print $n;}' | sort  > $dir/valid_uttlist
  rm $dir/uniq2utt $dir/valid_uttlist.tmp
fi

awk '{print $1}' $data/utt2spk | subtools/kaldi/utils/filter_scp.pl --exclude $dir/valid_uttlist | \
   subtools/kaldi/utils/shuffle_list.pl | head -$num_utts_subset > $dir/train_subset_uttlist || exit 1;
...
case $feat_type in
  raw) feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl --exclude $dir/valid_uttlist $sdata/JOB/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- |"
    valid_feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/valid_uttlist $data/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- |"
    train_subset_feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/train_subset_uttlist $data/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- |"
  ...
  lda)
    ...
    feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl --exclude $dir/valid_uttlist $sdata/JOB/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
    valid_feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/valid_uttlist $data/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
    train_subset_feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/train_subset_uttlist $data/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
    ;;
...
if [ ! -z "$online_ivector_dir" ]; then
  ...
  feats="$feats paste-feats --length-tolerance=$ivector_period ark:- 'ark,s,cs:subtools/kaldi/utils/filter_scp.pl $sdata/JOB/utt2spk $online_ivector_dir/ivector_online.scp | subsample-feats --n=-$ivector_period scp:- ark:- | ivector-randomize --randomize-prob=$ivector_randomize_prob ark:- ark:- |' ark:- |"
  valid_feats="$valid_feats paste-feats --length-tolerance=$ivector_period ark:- 'ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/valid_uttlist $online_ivector_dir/ivector_online.scp | subsample-feats --n=-$ivector_period scp:- ark:- | ivector-randomize --randomize-prob=$ivector_randomize_prob ark:- ark:- |' ark:- |"
  train_subset_feats="$train_subset_feats paste-feats --length-tolerance=$ivector_period ark:- 'ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/train_subset_uttlist $online_ivector_dir/ivector_online.scp | subsample-feats --n=-$ivector_period scp:- ark:- | ivector-randomize --randomize-prob=$ivector_randomize_prob ark:- ark:- |' ark:- |"
fi

if [ $stage -le 0 ]; then
  echo "$0: working out number of frames of training data"
  num_frames=$(subtools/kaldi/steps/nnet2/get_num_frames.sh $data)
...
# the subdirectory $dir/egs/storage/ exists.  See subtools/kaldi/utils/create_split_dir.pl
for x in `seq 1 $num_jobs_nnet`; do
  for y in `seq 0 $[$iters_per_epoch-1]`; do
    subtools/kaldi/utils/create_data_link.pl $dir/egs/egs.$x.$y.ark
    subtools/kaldi/utils/create_data_link.pl $dir/egs/egs_tmp.$x.$y.ark
  done
  for y in `seq 1 $nj`; do
    subtools/kaldi/utils/create_data_link.pl $dir/egs/egs_orig.$x.$y.ark
  done
done

remove () { for x in $*; do [ -L $x ] && rm $(subtools/kaldi/utils/make_absolute.sh $x); rm $x; done }
...
if [ $stage -le 2 ]; then
  ...
  for id in $(seq $nj); do gunzip -c $alidir/ali.$id.gz; done | \
    ...
    subtools/kaldi/utils/filter_scp.pl <(cat $dir/valid_uttlist $dir/train_subset_uttlist) | \

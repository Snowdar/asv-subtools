...
if [ ! -z "$online_ivector_dir" ]; then
  ...
  feats="$feats paste-feats --length-tolerance=$ivector_period ark:- 'ark,s,cs:subtools/kaldi/utils/filter_scp.pl $sdata/JOB/utt2spk $online_ivector_dir/ivector_online.scp | subsample-feats --n=-$ivector_period scp:- ark:- |' ark:- |"
  valid_feats="$valid_feats paste-feats --length-tolerance=$ivector_period ark:- 'ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/valid_uttlist $online_ivector_dir/ivector_online.scp | subsample-feats --n=-$ivector_period scp:- ark:- |' ark:- |"
  train_subset_feats="$train_subset_feats paste-feats --length-tolerance=$ivector_period ark:- 'ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/train_subset_uttlist $online_ivector_dir/ivector_online.scp | subsample-feats --n=-$ivector_period scp:- ark:- |' ark:- |"
else
...
# the subdirectory $dir/storage/ exists.  See subtools/kaldi/utils/create_split_dir.pl
for x in `seq $num_archives`; do
  subtools/kaldi/utils/create_data_link.pl $dir/egs.$x.ark
  for y in `seq $nj`; do
    subtools/kaldi/utils/create_data_link.pl $dir/egs_orig.$x.$y.ark
  done
done
...
if [ $stage -le 5 ]; then
  ...
  for x in `seq $num_archives`; do
    for y in `seq $nj`; do
      ...
      [ -L $file ] && rm $(subtools/kaldi/utils/make_absolute.sh $file)
      rm $file
    done
  done
fi

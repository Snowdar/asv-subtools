...
align_cmd=              # The cmd that is passed to subtools/kaldi/steps/nnet2/align.sh
align_use_gpu=          # Passed to use_gpu in subtools/kaldi/steps/nnet2/align.sh [yes/no]
...
if [ $# != 4 ]; then
  ...
  echo "  --align-cmd (subtools/kaldi/utils/run.pl|subtools/kaldi/utils/queue.pl <queue opts>) # passed to align.sh"
...
python subtools/kaldi/steps/nnet2/make_multisplice_configs.py contexts --splice-indexes "$splice_indexes" $dir || exit -1;
...
if [ $stage -le -4 ]; then
  ...
  subtools/kaldi/steps/nnet2/get_lda.sh $lda_opts "${extra_opts[@]}" --left-context $first_left_context --right-context $first_right_context --cmd "$cmd" $data $lang $alidir $dir || exit 1;
fi
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/kaldi/steps/nnet2/get_egs2.sh $egs_opts "${extra_opts[@]}" \
...
if [ $stage -le -2 ]; then
  ...
  python subtools/kaldi/steps/nnet2/make_multisplice_configs.py  \
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      subtools/kaldi/steps/nnet2/align.sh --nj $num_jobs_align --cmd "$align_cmd" --use-gpu $align_use_gpu \
        --transform-dir "$transform_dir" --online-ivector-dir "$online_ivector_dir" \
        --iter $x $data $lang $dir $dir/ali_$epoch || exit 1

      subtools/kaldi/steps/nnet2/relabel_egs2.sh --cmd "$cmd" --iter $x $dir/ali_$epoch \
        $prev_egs_dir $cur_egs_dir || exit 1

      if $cleanup && [[ $prev_egs_dir =~ $dir/egs* ]]; then
        subtools/kaldi/steps/nnet2/remove_egs.sh $prev_egs_dir
      fi
    fi
...
if $cleanup; then
  ...
  if [[ $cur_egs_dir =~ $dir/egs* ]]; then
    subtools/kaldi/steps/nnet2/remove_egs.sh $cur_egs_dir
  fi

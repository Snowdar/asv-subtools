...
if [ -z "$degs_dir" ] && [ -d $dir/degs/storage ]; then
  echo "$0: creating data links for distributed storage of degs"
    # See subtools/kaldi/utils/create_split_dir.pl for how this 'storage' directory
    # is created.
  for x in $(seq $num_jobs_nnet); do
    for y in $(seq $nj); do
      subtools/kaldi/utils/create_data_link.pl $dir/degs/degs_orig.$x.$y.ark
    done
    for z in $(seq 0 $[$iters_per_epoch-1]); do
      subtools/kaldi/utils/create_data_link.pl $dir/degs/degs_tmp.$x.$z.ark
      subtools/kaldi/utils/create_data_link.pl $dir/degs/degs.$x.$z.ark
    done
  done
fi

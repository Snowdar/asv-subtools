...
if [ $# != 6 ]; then
  ...
  echo "                                                   # subtools/kaldi/steps/online/nnet2/get_egs_discriminative2.sh instead)"
  exit 1;
fi
...
if [ $nj_ali -eq $nj ]; then
  ...
  prior_ali_rspecifier="ark,s,cs:gunzip -c $alis | copy-int-vector ark:- ark,t:- | subtools/kaldi/utils/filter_scp.pl $dir/priors_uttlist | ali-to-pdf $alidir/final.mdl ark,t:- ark:- |"
else
...
case $feat_type in
  ...
  lda)
    ...
    priors_feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/priors_uttlist $data/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $dir/final.mat ark:- ark:- |"
    ;;
...
if [ ! -z $online_ivector_dir ]; then
  ...
  priors_feats="$priors_feats paste-feats --length-tolerance=$ivector_period ark:- 'ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/priors_uttlist $online_ivector_dir/ivector_online.scp | subsample-feats --n=-$ivector_period scp:- ark:- |' ark:- |"
fi
...
if [ -d $dir/storage ]; then
  ...
  # See subtools/kaldi/utils/create_split_dir.pl for how this 'storage' directory is created.
  for x in $(seq $nj); do
    for y in $(seq $num_archives_temp); do
      subtools/kaldi/utils/create_data_link.pl $dir/degs_orig.$x.$y.ark
    done
  done
  for z in $(seq $num_archives); do
    subtools/kaldi/utils/create_data_link.pl $dir/degs.$z.ark
  done
  if [ $num_archives_temp -ne $num_archives ]; then
    for z in $(seq $num_archives); do
      subtools/kaldi/utils/create_data_link.pl $dir/degs_temp.$z.ark
    done
  fi
fi
...
if $cleanup; then
  ...
  for x in $(seq $nj); do
    for y in $(seq $num_archives_temp); do
      ...
      [ -L $file ] && rm $(subtools/kaldi/utils/make_absolute.sh $file); rm $file
    done
  done
  if [ $num_archives_temp -ne $num_archives ]; then
    for z in $(seq $num_archives); do

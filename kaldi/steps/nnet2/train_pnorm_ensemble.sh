...
if [ $stage -le -4 ]; then
  ...
  subtools/kaldi/steps/nnet2/get_lda.sh $lda_opts --splice-width $splice_width --cmd "$cmd" $data $lang $alidir $dir || exit 1;
fi
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/kaldi/steps/nnet2/get_egs.sh --samples-per-iter $samples_per_iter --num-jobs-nnet $num_jobs_nnet \

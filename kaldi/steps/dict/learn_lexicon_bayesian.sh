...
# half-learned lexicon using subtools/kaldi/steps/dict/apply_lexicon_edits to produce the final
...
if [ $stage -le 2 ]; then
  ...
  subtools/kaldi/utils/prepare_lang.sh --phone-symbol-table $ref_lang/phones.txt $dir/dict_expanded_train "$oov_symbol" \
...
if [ $stage -le 3 ]; then
  ...
  subtools/kaldi/steps/cleanup/debug_lexicon.sh --nj $nj --cmd "$decode_cmd" $data $dir/lang_expanded_train \
...
if [ $stage -le 4 ]; then
  ...
  if $retrain_src_mdl; then mdl_dir=$dir/${src_mdl_dir}_retrained; else mdl_dir=$src_mdl_dir; fi
  subtools/kaldi/steps/align_fmllr_lats.sh --acoustic-scale 0.05 --cmd "$decode_cmd" --nj $nj \
...
if [ $stage -le 5 ]; then
  ...
  if $retrain_src_mdl; then mdl_dir=$dir/${src_mdl_dir}_retrained; else mdl_dir=$src_mdl_dir; fi
  subtools/kaldi/steps/align_fmllr_lats.sh --cmd "$decode_cmd" --nj $nj \
...
if [ $stage -le 6 ]; then
  ...
  $cmd $dir/lats_iter2/log/select_prons_bayesian.log \
    subtools/kaldi/steps/dict/select_prons_bayesian.py --prior-mean=$prior_mean --prior-counts-tot=$prior_counts_tot \

...
# This recipe has similar inputs and outputs as subtools/kaldi/steps/dict/learn_lexicon.sh
...
# framework (see subtools/kaldi/steps/dict/select_prons_greedy.sh for details) to select an
...
# The output of subtools/kaldi/steps/dict/select_prons_greedy.sh is a learned lexicon whose vocab 
...
# half-learned lexicon using subtools/kaldi/steps/dict/apply_lexicon_edits.sh to produce the 
...
if [ $# -lt 6 ] || [ $# -gt 7 ]; then
  ...
  echo "                                      # the first iteration of lattice generation. See subtools/kaldi/steps/dict/internal/prune_pron_candidates.py"
  echo "                                      # for details."
  ...
  echo "                                      # see subtools/kaldi/steps/dict/select_prons_greedy.py for details."
...
if [ $stage -le 0 ]; then
  ...
  subtools/kaldi/steps/cleanup/internal/get_non_scored_words.py $ref_lang > $dir/non_scored_words
  ...
  $cmd $dir/log/prune_g2p_lexicon.log subtools/kaldi/steps/dict/prons_to_lexicon.py \
...
if [ $stage -le 1 ] && $retrain_src_mdl; then
  ...
  subtools/kaldi/utils/prepare_lang.sh --phone-symbol-table $ref_lang/phones.txt $dir/dict_expanded_target_vocab \
  ...
  subtools/kaldi/steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
  ...
  subtools/kaldi/steps/train_sat.sh --cmd "$train_cmd" $num_leaves $num_gauss \
...
if [ $stage -le 2 ]; then
  ...
  subtools/kaldi/utils/prepare_lang.sh $dir/dict_expanded_train $oov_symbol \
...
if [ $stage -le 3 ]; then
  ...
  subtools/kaldi/steps/cleanup/debug_lexicon.sh  --nj $nj \
...
if [ $stage -le 4 ]; then
  ...
  $cmd $dir/phonetic_decoding/log/prons_to_lexicon.log subtools/kaldi/steps/dict/prons_to_lexicon.py \
  ...
  subtools/kaldi/utils/prepare_lang.sh --phone-symbol-table $ref_lang/phones.txt \
  ...
  subtools/kaldi/steps/dict/internal/get_subsegments.py $dir/phonetic_decoding/word.ctm $dir/learn_vocab.txt \
    $dir/resegmentation/subsegments $dir/resegmentation/text || exit 1;
  subtools/kaldi/utils/data/subsegment_data_dir.sh $data $dir/resegmentation/subsegments $dir/resegmentation/text \
    $dir/resegmentation/data || exit 1;
  subtools/kaldi/steps/compute_cmvn_stats.sh $dir/resegmentation/data || exit 1;

  subtools/kaldi/steps/align_fmllr_lats.sh --beam 20 --retry-beam 50 --final-beam 30 --acoustic-scale 0.05 --cmd "$decode_cmd" --nj $nj \
  ...
  $cmd JOB=1:$nj $dir/lats_iter1/log/get_arc_info.JOB.log \
    ...
    subtools/kaldi/utils/int2sym.pl -f 5 $dir/lang_combined_iter1/words.txt \| \
    subtools/kaldi/utils/int2sym.pl -f 6- $dir/lang_combined_iter1/phones.txt '>' \
  ...
  cat $dir/lats_iter1/arc_info_sym.*.txt | subtools/kaldi/steps/dict/get_pron_stats.py - \
  ...
  # from roughly the same location. See subtools/kaldi/steps/dict/internal/sum_arc_info.py for details.
  for i in `seq 1 $nj`;do
    cat $dir/lats_iter1/arc_info_sym.${i}.txt | sort -n -k1 -k2 -k3r | \
      subtools/kaldi/steps/dict/internal/sum_arc_info.py - $dir/phonetic_decoding/phone_map.txt $dir/lats_iter1/arc_info_summed.${i}.txt
  done 
...
if [ $stage -le 5 ]; then
  ...
  $cmd $dir/lats_iter1/log/prune_pron_candidates.log subtools/kaldi/steps/dict/internal/prune_pron_candidates.py \
  ...
  if $retrain_src_mdl; then mdl_dir=$dir/${src_mdl_dir}_retrained; else mdl_dir=$src_mdl_dir; fi
  subtools/kaldi/steps/align_fmllr_lats.sh  --beam 30 --retry-beam 60 --final-beam 50 --acoustic-scale 0.05 --cmd "$decode_cmd" --nj $nj \
  ...
  $cmd JOB=1:$nj $dir/lats_iter2/log/get_arc_info.JOB.log \
    ...
    subtools/kaldi/utils/int2sym.pl -f 5 $dir/lang_combined_iter2/words.txt \| \
    subtools/kaldi/utils/int2sym.pl -f 6- $dir/lang_combined_iter2/phones.txt '>' \
  ...
  cat $dir/lats_iter2/arc_info_sym.*.txt | subtools/kaldi/steps/dict/get_pron_stats.py - \
  ...
  for i in `seq 1 $nj`;do
    cat $dir/lats_iter2/arc_info_sym.${i}.txt | sort -n -k1 -k2 -k3r | \
      subtools/kaldi/steps/dict/internal/sum_arc_info.py - $dir/phonetic_decoding/phone_map.txt $dir/lats_iter2/arc_info_summed.${i}.txt
  done 
  ...
  subtools/kaldi/utils/split_scp.pl --utt2spk=$dir/lats_iter2/utt2word $dir/lats_iter2/utt2word $utt2words || exit 1
...
if [ $stage -le 6 ]; then
  ...
  $cmd JOB=1:$nj_select_prons $dir/lats_iter2/log/generate_learned_lexicon.JOB.log \
    subtools/kaldi/steps/dict/select_prons_greedy.py \
  ...
  $cmd $dir/lats_iter2/log/lexicon_learning_summary.log \
    subtools/kaldi/steps/dict/merge_learned_lexicons.py \
...
if [ $stage -le 7 ]; then
  ...
  # this part, later one we'll apply recommended changes using subtools/kaldi/steps/dict/apply_lexicon_edits.py
...
if [ $stage -le 8 ]; then
  ...
  echo "  ... subtools/kaldi/steps/dict/apply_lexicon_edits.py $dest_dict/lexicon0.txt $dir/lats_iter2/ref_lexicon_edits.txt  - | \\"
  ...
  subtools/kaldi/steps/dict/apply_lexicon_edits.py $dest_dict/lexicon0.txt $dir/lats_iter2/ref_lexicon_edits.txt - | \

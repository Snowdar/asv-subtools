...
if [ $# -ne 4 ]; then
  ...
  echo "subtools/kaldi/steps/cleanup/cleanup_with_segmentation.sh."
  ...
  echo "Note: most of this is the same as is done in subtools/kaldi/steps/cleanup/find_bad_utts.sh,"
...
if [ $stage -le 1 ]; then
  $cmd JOB=1:$nj $dir/log/get_oracle.JOB.log \
    ...
    "ark:subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt $sdata/JOB/text|" \
    ark,t:- \| subtools/kaldi/utils/int2sym.pl -f 2- $lang/words.txt '>' $dir/oracle_hyp.JOB.txt || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  if [ -f $lang/phones/word_boundary.int ]; then
    $cmd JOB=1:$nj $dir/log/get_ctm.JOB.log \
      ...
      subtools/kaldi/utils/int2sym.pl -f 5 $lang/words.txt '>' $dir/ctm.JOB || exit 1;
...
if [ $stage -le 3 ]; then
  ...
  subtools/kaldi/utils/filter_scp.pl $dir/oracle_hyp.txt < $data/text  > $dir/text
...
if [ $stage -le 4 ]; then
  ...
  cat $dir/analysis/per_utt_details.txt | \
    subtools/kaldi/utils/scoring/wer_ops_details.pl --special-symbol "$special_symbol" | \
...
if [ $stage -le 5 ]; then
  ...
  $cmd $dir/log/get_ctm_edits.log \
    align-text ark:$dir/oracle_hyp.txt ark:$dir/text ark,t:-  \| \
      subtools/kaldi/steps/cleanup/internal/get_ctm_edits.py --oov=$oov --symbol-table=$lang/words.txt \

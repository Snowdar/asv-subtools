...
subtools/kaldi/utils/lang/check_phones_compatible.sh $lang/phones.txt $dir/phones.txt
...
if [ $stage -le 0 ]; then
  ...
  set +o pipefail
  subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt < $text | \
  ...
  subtools/kaldi/utils/int2sym.pl -f 1 $lang/words.txt <$graph_dir/top_words.int >$graph_dir/top_words.txt
fi
...
subtools/kaldi/utils/split_scp.pl $text $split_text
...
if [ $stage -le 1 ]; then
  ...
  $cmd JOB=1:$nj $graph_dir/log/compile_decoding_graphs.JOB.log \
    subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt $graph_dir/texts/text.JOB \| \
    subtools/kaldi/steps/cleanup/make_biased_lms.py --min-words-per-graph=$min_words_per_graph \
...
subtools/kaldi/utils/apply_map.pl -f 2 $graph_dir/fsts/HCLG.fsts.per_utt.scp <$graph_dir/fsts/utt2group > $graph_dir/HCLG.fsts.scp

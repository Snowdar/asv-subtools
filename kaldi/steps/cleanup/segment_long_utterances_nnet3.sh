...
# This script is similar to subtools/kaldi/steps/cleanup/segment_long_utterances.sh, but
...
# subtools/kaldi/steps/cleanup/clean_and_segment_data_nnet3.sh on the output in order to
...
# subtools/kaldi/steps/cleanup/internal/segment_ctm_edits_mild.py
...
subtools/kaldi/steps/cleanup/clean_and_segment_data.sh on the output in order to further clean
it and eliminate data where the transcript doesn't seem to match.
  main options (for others, see top of script file):
    ...
    --graph-opts 'opts'         # Additional options to make_biased_lm_graphs.sh.
                                # Please run subtools/kaldi/steps/cleanup/make_biased_lm_graphs.sh
                                # without arguments to see allowed options.
    --segmentation-extra-opts 'opts'  # Additional options to segment_ctm_edits_mild.py.
                                # Please run subtools/kaldi/steps/cleanup/internal/segment_ctm_edits_mild.py
                                # without arguments to see allowed options.
...
subtools/kaldi/utils/lang/check_phones_compatible.sh $lang/phones.txt $srcdir/phones.txt
...
if [ $stage -le 2 ]; then
  ...
  if [ ! -z "$seconds_per_spk_max" ]; then
    subtools/kaldi/utils/data/subsegment_data_dir.sh \
      $data $dir/uniform_sub_segments $dir/${data_id}_uniform_seg.temp

    subtools/kaldi/utils/data/modify_speaker_info.sh --seconds-per-spk-max $seconds_per_spk_max \
  ...
  subtools/kaldi/utils/fix_data_dir.sh $data_uniform_seg

  # Compute new cmvn stats for the segmented data directory
  subtools/kaldi/steps/compute_cmvn_stats.sh $data_uniform_seg/
fi
...
if [ $stage -le 3 ]; then
  ...
  subtools/kaldi/steps/cleanup/make_biased_lm_graphs.sh $graph_opts \
  ...
  if [ -z "$utt2text" ]; then
    ...
    cat $dir/uniform_sub_segments | awk '{print $1" "$2}' | \
      subtools/kaldi/utils/apply_map.pl -f 2 $dir/graphs/HCLG.fsts.scp | \
      sort -k1,1 > \
      $graph_dir/HCLG.fsts.scp
  else
    # and then copy it to the sub-segments.
    cat $dir/uniform_sub_segments | awk '{print $1" "$2}' | \
      subtools/kaldi/utils/apply_map.pl -f 2 $utt2text | \
      subtools/kaldi/utils/apply_map.pl -f 2 $dir/graphs/HCLG.fsts.scp | \
      sort -k1,1 > \
      $graph_dir/HCLG.fsts.scp
  fi
...
if [ ! -z "$extractor" ]; then
  ...
  if [ $stage -le 4 ]; then
    ...
    if $use_vad; then
      subtools/kaldi/steps/compute_vad_decision.sh $data_uniform_seg \
        $data_uniform_seg/log $data_uniform_seg/data
    fi

    subtools/kaldi/steps/online/nnet2/extract_ivectors_online.sh \
...
if [ $stage -le 5 ]; then
  ...
  subtools/kaldi/steps/cleanup/decode_segmentation_nnet3.sh \
    --acwt $acwt \
...
if [ $stage -le 6 ]; then
  subtools/kaldi/steps/get_ctm_fast.sh --lmwt $lmwt --cmd "$cmd --mem 4G" \
...
if [ $stage -le 7 ]; then
  ...
  subtools/kaldi/steps/cleanup/internal/split_text_into_docs.pl --max-words $max_words \
    $text $dir/docs/doc2text $dir/docs/docs.txt
  subtools/kaldi/utils/utt2spk_to_spk2utt.pl $dir/docs/doc2text > $dir/docs/text2doc
fi
...
if [ $stage -le 8 ]; then
  ...
  subtools/kaldi/utils/split_data.sh $data_uniform_seg $nj

  mkdir -p $dir/docs/split$nj/

  # First compute IDF stats
  $cmd $dir/log/compute_source_idf_stats.log \
    subtools/kaldi/steps/cleanup/internal/compute_tf_idf.py \
  ...
  for n in `seq $nj`; do
    ...
    subtools/kaldi/utils/filter_scp.pl $data_uniform_seg/split$nj/$n/utt2spk $dir/uniform_sub_segments | \
      cut -d ' ' -f 1,2 | subtools/kaldi/utils/utt2spk_to_spk2utt.pl > $sdir/old2new_utts.$n.txt
    ...
    if [ ! -z "$utt2text" ]; then
      ...
      # split.
      subtools/kaldi/utils/filter_scp.pl $sdir/old2new_utts.$n.txt $utt2text | \
        cut -d ' ' -f 2 | sort -u | \
        subtools/kaldi/utils/filter_scp.pl /dev/stdin $dir/docs/text2doc > $sdir/text2doc.$n
    else
      subtools/kaldi/utils/filter_scp.pl $sdir/old2new_utts.$n.txt \
        $dir/docs/text2doc > $sdir/text2doc.$n
    fi

    subtools/kaldi/utils/spk2utt_to_utt2spk.pl $sdir/text2doc.$n | \
      subtools/kaldi/utils/filter_scp.pl /dev/stdin $dir/docs/docs.txt > \
      $sdir/docs.$n.txt
  done
...
if [ $stage -le 9 ]; then
  ...
  subtools/kaldi/steps/cleanup/internal/get_non_scored_words.py $lang > $dir/non_scored_words.txt
fi
...
if [ $stage -le 10 ]; then
  ...
  $cmd JOB=1:$nj $decode_dir/ctm_$lmwt/log/compute_query_tf_idf.JOB.log \
    subtools/kaldi/steps/cleanup/internal/ctm_to_text.pl --non-scored-words $dir/non_scored_words.txt \
  ...
  $cmd JOB=1:$nj $dir/log/retrieve_similar_docs.JOB.log \
    subtools/kaldi/steps/cleanup/internal/retrieve_similar_docs.py \
  ...
  $cmd JOB=1:$nj $decode_dir/ctm_$lmwt/log/get_ctm_edits.JOB.log \
    subtools/kaldi/steps/cleanup/internal/stitch_documents.py \
      --query2docs=$dir/query_docs/split$nj/relevant_docs.JOB.txt \
      --input-documents=$dir/docs/split$nj/docs.JOB.txt \
      --output-documents=- \| \
    subtools/kaldi/steps/cleanup/internal/align_ctm_ref.py --eps-symbol='"<eps>"' \
...
if [ $stage -le 11 ]; then
  $cmd $dir/log/resolve_ctm_edits.log \
    subtools/kaldi/steps/cleanup/internal/resolve_ctm_edits_overlaps.py \
...
if [ $stage -le 12 ]; then
  ...
  $cmd $dir/log/modify_ctm_edits.log \
    subtools/kaldi/steps/cleanup/internal/modify_ctm_edits.py --verbose=3 $dir/non_scored_words.txt \
...
if [ $stage -le 13 ]; then
  ...
  $cmd $dir/log/taint_ctm_edits.log \
       subtools/kaldi/steps/cleanup/internal/taint_ctm_edits.py --remove-deletions=false \
...
mkdir -p $out_data
if [ $stage -le 15 ]; then
  subtools/kaldi/utils/data/subsegment_data_dir.sh $data_uniform_seg \

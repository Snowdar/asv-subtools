...
if [ -z $alidir ]; then
  ...
  if [ $stage -le 1 ]; then
    subtools/kaldi/steps/align_fmllr.sh --cmd "$cmd" --nj $nj $data $lang $src $alidir
  fi
fi
...
if [ $stage -le 2 ]; then
  subtools/kaldi/utils/lang/make_phone_bigram_lang.sh $lang $alidir $phone_lang
fi

if [ $stage -le 3 ]; then
  subtools/kaldi/utils/mkgraph.sh $phone_lang $src $src/graph_phone_bg
fi

if [ $stage -le 4 ]; then
  subtools/kaldi/steps/decode_si.sh --skip-scoring true \
...
if [ $stage -le 5 ]; then
  subtools/kaldi/steps/get_train_ctm.sh --print-silence true --use-segments false \
     --cmd "$cmd" $data $lang $alidir
fi

if [ $stage -le 6 ]; then
  subtools/kaldi/steps/get_ctm.sh --use-segments false --cmd "$cmd" --min-lmwt 3 --max-lmwt 8 \
...
# work out for each word, what the phones were during that time.
  ...
  cat $dir/phone.ctm | subtools/kaldi/utils/apply_map.pl -f 5 $dir/phone_map.txt > $dir/phone_mapped.ctm
...
if [ $stage -le 9 ]; then
  ...
  subtools/kaldi/steps/cleanup/internal/get_non_scored_words.py $lang > $dir/non_scored_words
  subtools/kaldi/steps/cleanup/internal/get_pron_stats.py $dir/ctm_prons.txt $phone_lang/phones/silence.txt $phone_lang/phones/optional_silence.txt $dir/non_scored_words - | \

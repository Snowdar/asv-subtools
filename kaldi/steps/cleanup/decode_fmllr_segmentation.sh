...
# Similar to subtools/kaldi/steps/cleanup/decode_segmentation.sh, but does fMLLR adaptation.
...
subtools/kaldi/utils/lang/check_phones_compatible.sh $graphdir/phones.txt $srcdir/phones.txt
...
if [ -z "$si_dir" ]; then # we need to do the speaker-independent decoding pass.
  ...
  if [ $stage -le 0 ]; then
    ...
    subtools/kaldi/steps/cleanup/decode_segmentation.sh --scoring-opts "$scoring_opts" \

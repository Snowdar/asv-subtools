...
if [ $# != 4 ]; then
  ...
  echo "  ... please use subtools/kaldi/steps/cleanup/clean_and_segment_data.sh,"
...
if [ $stage -le 0 ]; then
  subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt <$data/text | \
  ...
  subtools/kaldi/utils/int2sym.pl -f 2 $lang/words.txt <$dir/top_words.int >$dir/top_words.txt
fi
...
if [ $stage -le 1 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/decode.JOB.log \
    subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt $sdata/JOB/text \| \
    subtools/kaldi/steps/cleanup/make_utterance_fsts.pl $dir/top_words.int \| \
    ...
    lattice-oracle ark:- "ark:subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt $sdata/JOB/text|" \
      ark,t:- ark,t:$dir/edits.JOB.txt \| \
    subtools/kaldi/utils/int2sym.pl -f 2- $lang/words.txt '>' $dir/aligned_ref.JOB.txt || exit 1;
fi
...
if [ $stage -le 2 ]; then
  ...
  subtools/kaldi/utils/filter_scp.pl $dir/edits.txt < $data/text  > $dir/text
...
if [ $stage -le 3 ]; then
  ...
  align-text --special-symbol="***"  ark:$dir/text ark:$dir/aligned_ref.txt  ark,t:- | \
    subtools/kaldi/utils/scoring/wer_per_utt_details.pl --special-symbol "***" > $dir/analysis/per_utt_details.txt

  cat $dir/analysis/per_utt_details.txt | \
    subtools/kaldi/utils/scoring/wer_per_spk_details.pl $data/utt2spk > $dir/analysis/per_spk_details.txt

  cat $dir/analysis/per_utt_details.txt | \
    subtools/kaldi/utils/scoring/wer_ops_details.pl --special-symbol "***" | \

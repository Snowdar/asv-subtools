...
if [ $# -ne 5 ]; then
  ...
  main options (for others, see top of script file):
    ...
    --segmentation-opts 'opts'  # Additional options to segment_ctm_edits.py.
                                # Please run subtools/kaldi/steps/cleanup/internal/segment_ctm_edits.py
                                # without arguments to see allowed options.
...
if [ ! -z "$extractor" ]; then
  ...
  if [ $stage -le 2 ]; then
    ...
    if $use_vad; then
      subtools/kaldi/steps/compute_vad_decision.sh $data \
        $data/log $data/data
    fi
...
if [ $stage -le 3 ]; then
  ...
  subtools/kaldi/steps/diagnostic/analyze_lats.sh --cmd "$cmd" $lang $dir/lats
fi
...
if [ $stage -le 4 ]; then
  ...
  subtools/kaldi/steps/cleanup/lattice_oracle_align.sh --cmd "$cmd --mem 4G" $frame_shift_opt \
...
if [ $stage -le 6 ]; then
  ...
  $cmd $dir/log/taint_ctm_edits.log \
       subtools/kaldi/steps/cleanup/internal/taint_ctm_edits.py $dir/ctm_edits.modified $dir/ctm_edits.tainted
...
if [ $stage -le 7 ]; then
  ...
  $cmd $dir/log/segment_ctm_edits.log \
    subtools/kaldi/steps/cleanup/internal/segment_ctm_edits.py \
      $segmentation_opts \
...
if [ $stage -le 8 ]; then
  ...
  subtools/kaldi/utils/apply_map.pl -f 1 <(awk '{print $1,$1,$2}' <$data/utt2dur) <$dir/utt2dur.from_ctm  | \
...
if [ $stage -le 8 ]; then
  ...
  subtools/kaldi/utils/data/subsegment_data_dir.sh --segment-end-padding $padding ${data} $dir/segments $dir/text $data_out
  # subtools/kaldi/utils/data/subsegment_data_dir.sh can output directories that have e.g. to many entries left in wav.scp
...
if [ $stage -le 9 ]; then
  ...
  subtools/kaldi/steps/compute_cmvn_stats.sh $data_out $data_out/log $data_out/data
fi

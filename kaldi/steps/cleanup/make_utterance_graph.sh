...
if [ $# -ne 4 ]; then
  ...
  echo "then subtools/kaldi/utils/make_unigram_grammar.pl will be used to build the unigram"
...
subtools/kaldi/utils/lang/check_phones_compatible.sh $lang/phones.txt $model_dir/phones.txt
...
cat $text | subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 2- $lang/words.txt | \
 subtools/kaldi/utils/int2sym.pl -f 2- $lang/words.txt | \
 while read line; do
  ...
  if [ $ngram_order -eq 1 ]; then
    ...
    cat $wdir/text | subtools/kaldi/utils/sym2int.pl --map-oov $oov -f 1- $lang/words.txt | \
      subtools/kaldi/utils/make_unigram_grammar.pl | fstcompile |\

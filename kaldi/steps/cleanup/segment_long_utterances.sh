...
# subtools/kaldi/steps/cleanup/clean_and_segment_data.sh on the output in order to
...
frame_shift=`subtools/kaldi/utils/data/get_frame_shift.sh $data`
...
if [ $stage -le 4 ]; then
  ...
  if [ -f $srcdir/trans.1 ]; then
    subtools/kaldi/steps/cleanup/decode_fmllr_segmentation.sh \
    ...
    subtools/kaldi/steps/cleanup/decode_segmentation.sh \
      --beam $beam --lattice-beam $lattice_beam --nj $nj --cmd "$cmd --mem 4G" \
      --skip-scoring true --allow-partial false \
      $graph_dir $data_uniform_seg $decode_dir
  fi
...
if [ $stage -le 5 ]; then
  subtools/kaldi/steps/get_ctm_fast.sh --frame_shift $frame_shift --lmwt $lmwt --cmd "$cmd --mem 4G" \
    --print-silence true \

...
if ! $skip_phoneset_check; then
  subtools/kaldi/utils/lang/check_phones_compatible.sh $lang/phones.txt $alidir/phones.txt
  subtools/kaldi/utils/lang/check_phones_compatible.sh $lang/phones.txt $alidir_cv/phones.txt
...
# shuffle the list,
subtools/kaldi/utils/shuffle_list.pl --srand ${seed:-777} <$dir/train_sorted.scp >$dir/train.scp
...
if [ -n "$split_feats" ]; then
  ...
  subtools/kaldi/utils/split_scp.pl $dir/train.scp $scps
fi

# for debugging, add lists with non-local features,
subtools/kaldi/utils/shuffle_list.pl --srand ${seed:-777} <$data/feats.scp >$dir/train.scp_non_local
...
if [ -n "$feature_transform" ]; then
  ...
  case $feat_type in
    ...
    traps)
      ...
      subtools/kaldi/utils/nnet/gen_hamm_mat.py --fea-dim=$feat_dim --splice=$splice > $dir/hamm.mat
      subtools/kaldi/utils/nnet/gen_dct_mat.py --fea-dim=$feat_dim --splice=$splice --dct-basis=$traps_dct_basis > $dir/dct.mat
    ...
    transf)
      ...
      nnet-concat --binary=false $feature_transform_old \
        ...
        "subtools/kaldi/utils/nnet/gen_splice.py --fea-dim=$feat_dim --splice=$splice_after_transf |" \
        $feature_transform
    ;;
    *)
...
if [ ! -z $nnet_init ]; then
  ...
  case "$network_type" in
    dnn)
      subtools/kaldi/utils/nnet/make_nnet_proto.py $proto_opts \
    ...
    cnn1d)
      ...
      subtools/kaldi/utils/nnet/make_cnn_proto.py $cnn_proto_opts \
      ...
      ;;
    lstm)
      subtools/kaldi/utils/nnet/make_lstm_proto.py $proto_opts \
        $num_fea $num_tgt >$nnet_proto
      ;;
    blstm)
      subtools/kaldi/utils/nnet/make_blstm_proto.py $proto_opts \
        $num_fea $num_tgt >$nnet_proto
      ;;
...
subtools/kaldi/steps/nnet/train_scheduler.sh \
  ${scheduler_opts} \

...
# gunzip -c prons.*.gz | subtools/kaldi/utils/sym2int.pl -f 4 words.txt | subtools/kaldi/utils/sym2int.pl -f 5- phones.txt
...
if [ $stage -le 3 ]; then
  cat $dir/pron_counts.int | subtools/kaldi/utils/int2sym.pl -f 2 $lang/words.txt | \
    subtools/kaldi/utils/int2sym.pl -f 3- $lang/phones.txt | sort -nr > $dir/pron_counts.txt
fi
...
if [ $stage -le 5 ]; then
  ...
  gunzip -c $dir/prons.*.gz | subtools/kaldi/utils/int2sym.pl -f 4 $lang/words.txt | \
    subtools/kaldi/utils/int2sym.pl -f 5- $lang/phones.txt | cut -d ' ' -f 1,4- | awk '
    BEGIN { utter_id = ""; }
    {

...
if [ $stage -le 0 ]; then
  ...
  subtools/kaldi/utils/mkgraph.sh --self-loop-scale $self_loop_scale $new_lang $srcdir $dir/dengraph || exit 1;
fi
...
if [ -e $dir/storage ]; then
  ...
  subtools/kaldi/utils/create_data_link.pl $(for x in $(seq $num_archives); do echo $dir/degs.$x.scp; done)
  subtools/kaldi/utils/create_data_link.pl $(for y in $(seq $nj); do echo $dir/degs_orig.$y.ark; done)
  subtools/kaldi/utils/create_data_link.pl $(for y in $(seq $nj); do echo $dir/degs_orig.$y.scp; done)
  subtools/kaldi/utils/create_data_link.pl $(for y in $(seq $nj); do echo $dir/degs_orig_filtered.$y.scp; done)
fi
...
if [ $stage -le 3 ]; then
  ...
  $cmd $threads_cmd_opt JOB=1:$nj $dir/log/decode_and_get_egs.JOB.log \
    ...
    nnet3-discriminative-get-egs --acoustic-scale=$acwt --compress=$compress \
      ...
      "scp:subtools/kaldi/utils/filter_scp.pl $sdata/JOB/utt2spk $dir/ali.scp |" \
...
if [ $stage -le 4 ]; then
  ...
  done | subtools/kaldi/utils/shuffle_list.pl | head -n$num_utts_subset >$dir/valid_diagnostic.scp || exit 1
...
if [ $stage -le 6 ]; then
  ...
  # use subtools/kaldi/utils/shuffle_list.pl because it provides a complete shuffle (ok since
  ...
  done  | subtools/kaldi/utils/shuffle_list.pl | head -n$num_utts_subset >$dir/train_diagnostic.scp
...
if [ $stage -le 10 ] && $cleanup; then
  ...
  for j in $(seq $nj); do
    for f in $dir/degs_orig.$j.{ark,scp} $dir/degs_orig_filtered.$j.scp; do
      [ -L $f ] && rm $(subtools/kaldi/utils/make_absolute.sh $f); rm $f
    done
  done

...
jesus_opts=  # opts to subtools/kaldi/steps_multitask/nnet3/make_jesus_configs.py.
...
if [ $stage -le -5 ]; then
  ...
  if [ ! -z "$jesus_opts" ]; then
    $cmd $dir/log/make_configs.log \
       python subtools/kaldi/steps_multitask/nnet3/make_jesus_configs.py \
    ...
    python subtools/kaldi/steps_multitask/nnet3/tdnn/make_configs.py $pool_opts \
      --include-log-softmax=false \
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/kaldi/steps_multitask/nnet3/chain/get_egs.sh $egs_opts "${extra_opts[@]}" \
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    models_to_average=$(subtools/kaldi/steps_multitask/nnet3/get_successful_models.py --difference-threshold 0.1 $this_num_jobs $dir/log/train.$x.%.log)
    nnets_list=

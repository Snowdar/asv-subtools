...
if [ $stage -le -5 ]; then
  ...
  subtools/kaldi/steps_multitask/nnet3/lstm/make_configs.py  "${config_extra_opts[@]}" \
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/kaldi/steps_multitask/nnet3/get_egs.sh $egs_opts "${extra_opts[@]}" \
      --cmd "$cmd" $egs_opts \
      --stage $get_egs_stage \
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      subtools/kaldi/steps_multitask/nnet3/align.sh --nj $num_jobs_align --cmd "$align_cmd" --use-gpu $align_use_gpu \
        --transform-dir "$transform_dir" --online-ivector-dir "$online_ivector_dir" \
        --iter $x $data $lang $dir $dir/ali_$time || exit 1

      subtools/kaldi/steps_multitask/nnet3/relabel_egs.sh --cmd "$cmd" --iter $x $dir/ali_$time \
        $prev_egs_dir $cur_egs_dir || exit 1

      if $cleanup && [[ $prev_egs_dir =~ $dir/egs* ]]; then
        subtools/kaldi/steps_multitask/nnet3/remove_egs.sh $prev_egs_dir
      fi
    fi
    ...
    models_to_average=$(subtools/kaldi/steps_multitask/nnet3/get_successful_models.py $this_num_jobs $dir/log/train.$x.%.log)
    nnets_list=

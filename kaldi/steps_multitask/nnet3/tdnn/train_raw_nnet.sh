...
if [ -z "$online_ivector_dir" ]; then
  ...
  subtools/kaldi/steps/nnet2/get_ivector_id.sh $online_ivector_dir > $dir/final.ie.id || exit 1
fi
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/kaldi/steps_multitask/nnet3/get_egs_targets.sh $egs_opts "${extra_opts[@]}" \
...
if [ ! -z "$online_ivector_dir" ] ; then
  subtools/kaldi/steps/nnet2/check_ivectors_compatible.sh $online_ivector_dir $egs_dir/info || exit 1
fi

...
if [[ ( $# -lt 4 ) || ( $# -gt 6 ) ]]; then
   echo "usage: subtools/kaldi/steps_multitask/nnet3/make_bottleneck_features.sh <bnf-node-name> <input-data-dir> <bnf-data-dir> <nnet-dir> [<log-dir> [<bnfdir>] ]"
   echo "e.g.:  subtools/kaldi/steps_multitask/nnet3/make_bottleneck_features.sh tdnn_bn.renorm data/train data/train_bnf exp/nnet3/tdnn_bnf exp_bnf/dump_bnf bnf"
...
if [ ! -z "$ivector_dir" ];then
  use_ivector=true
  subtools/kaldi/steps/nnet2/check_ivectors_compatible.sh $nnetdir $ivector_dir || exit 1;
fi

feats="ark,s,cs:apply-cmvn $cmvn_opts --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:$sdata/JOB/feats.scp ark:- |"
ivector_feats="scp:subtools/kaldi/utils/filter_scp.pl $sdata/JOB/utt2spk $ivector_dir/ivector_online.scp |"


if [ $stage -le 1 ]; then
  echo "$0: Generating bottleneck features using $bnf_nnet model as output of "
  echo "    component-node with name $bnf_name."
  echo "output-node name=output.bnf input=$bnf_name" > $bnf_data/output.config.tmp
  ...
  modified_bnf_nnet="nnet3-copy --nnet-config=$bnf_data/output.config.tmp $bnf_nnet - | nnet3-copy --edits='remove-output-nodes name=output' - - | nnet3-copy --nnet-config=$bnf_data/output.config - - |  nnet3-copy --edits='remove-output-nodes name=output.bnf' - - |" 
#  nnet3-copy --nnet-config=$bnf_data/output.config.tmp $bnf_nnet - | nnet3-copy --edits='remove-output-nodes name=output' - - | nnet3-copy --nnet-config=$bnf_data/output.config - - |  nnet3-copy --edits='remove-output-nodes name=output.bnf' - test 
  ivector_opts=
  if $use_ivector; then
...
subtools/kaldi/steps/compute_cmvn_stats.sh $bnf_data

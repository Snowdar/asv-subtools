...
if [ $# != 6 ]; then
  ...
  echo "  --cmd (subtools/kaldi/utils/run.pl|subtools/kaldi/utils/queue.pl <queue opts>) # how to run jobs (probably would be good to add --max-jobs-run 5 or so if using"
...
prior_ali_rspecifier="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/priors_uttlist $dir/ali.scp | ali-to-pdf $alidir/final.mdl scp:- ark:- |"
...
awk '{print $1}' $data/utt2spk | subtools/kaldi/utils/filter_scp.pl --exclude $dir/valid_uttlist | \
  subtools/kaldi/utils/shuffle_list.pl | head -$num_priors_subset \
...
priors_feats="ark,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/priors_uttlist $data/feats.scp | apply-cmvn $cmvn_opts --utt2spk=ark:$data/utt2spk scp:$data/cmvn.scp scp:- ark:- |"
...
if [ -e $dir/storage ]; then
  ...
  subtools/kaldi/utils/create_data_link.pl $(for x in $(seq $num_archives); do echo $dir/degs.$x.ark; done)
  for x in $(seq $num_archives_intermediate); do
    subtools/kaldi/utils/create_data_link.pl $(for y in $(seq $nj); do echo $dir/degs_orig.$y.$x.ark; done)
  done
fi
...
(
  if $adjust_priors && [ $stage -le 10 ]; then
    ...
    for y in `seq $num_archives_priors`; do
      subtools/kaldi/utils/create_data_link.pl $dir/priors_egs.$y.ark
...
if [ $stage -le 5 ]; then
  ...
  $cmd --max-jobs-run $max_jobs_run JOB=1:$nj $dir/log/get_egs.JOB.log \
    nnet3-discriminative-get-egs $ivector_opts $egs_opts \
      ...
      "scp:subtools/kaldi/utils/filter_scp.pl $sdata/JOB/utt2spk $dir/ali.scp |" ark:- \| \

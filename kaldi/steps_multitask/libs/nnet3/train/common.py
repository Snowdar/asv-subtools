...
def get_egs_opts(egs_dir, egs_prefix="",
                           archive_index=-1,
                           use_egs=False):
    """ Generates egs option for multitask(or multilingual) training setup,
        ...
        e.g. Returns the empty string ('') if use_egs == False,
...
def halve_cvector_minibatch_size_str(minibatch_size_str):
    """Halve a minibatch-size string, as would be validated by
    validate_minibatch_size_str (see docs for that).  This halves
    all the integer elements of minibatch_size_str that represent minibatch
    sizes (as opposed to chunk-lengths) and that are >1."""
    ...
    new_minibatch_size = ''
    for minibatch_size in [am_minibatch_size, xvec_minibatch_size]:
        if not validate_minibatch_size_str(minibatch_size):
            raise Exception("Invalid minibatch-size string '{0}'".format(minibatch_size))
    
        a = minibatch_size.split("/")
        if len(a) == 1:
            new_minibatch_size += halve_range_str(a[0]) + ';'
            continue
        ...
        for elem in a:
            ...
            assert len(b) == 2
            ans.append('{0}={1}'.format(b[0], halve_range_str(b[1])))
        new_minibatch_size += '/'.join(ans) + ';'

    return new_minibatch_size[:-1]
...
def get_input_model_info(input_model):
    ...
    try:
        ...
        # but, if the net has two or more input, 
        # nnet3-info will not give information about
        # the context. Set it to 0 then
        if 'model_left_context' not in variables:
            variables['model_left_context'] = 0
            logger.warn('Cannot read left context from nnet3-info')
        if 'model_right_context' not in variables:
            variables['model_right_context'] = 0
            logger.warn('Cannot read right context from nnet3-info')
...
def prepare_initial_network(dir, run_opts, srand=-3, input_model=None):
    """ This function adds the first layer.
    ...
    if input_model is None:
        ...
        shutil.copyfile(input_model, "{dir}/0.raw".format(dir=dir))
...
def should_do_shrinkage(iter, model_file, shrink_saturation_threshold,
    ...
    if get_raw_nnet_from_am:
        output = common_lib.get_command_stdout(
            ...
            "subtools/kaldi/steps_multitask/nnet3/get_saturation.pl".format(model_file))
    else:
        output = common_lib.get_command_stdout(
...
def remove_nnet_egs(egs_dir):
    common_lib.execute_command("subtools/kaldi/steps/nnet2/remove_egs.sh {egs_dir}".format(
            egs_dir=egs_dir))
...
class CommonParser(object):
    ...
    in subtools/kaldi/steps_multitask/nnet3/train*.py and subtools/kaldi/steps_multitask/nnet3/chain/train.py
    """
    ...
    def __init__(self,
        ...
        self.parser.add_argument("--egs.transform_dir", type=str,
                                 ...
                                 "subtools/kaldi/steps_multitask/nnet3/get_egs.sh script")
        ...
        self.parser.add_argument("--egs.opts", type=str, dest='egs_opts',
                                 ...
                                 to subtools/kaldi/steps_multitask/nnet3/get_egs.sh script""")

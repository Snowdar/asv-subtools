...
def generate_chain_egs(dir, data, lat_dir, egs_dir,
    ...
    """Wrapper for subtools/kaldi/steps_multitask/nnet3/chain/get_egs.sh

    See options in that script.
    """

    common_lib.execute_command(
        """subtools/kaldi/steps_multitask/nnet3/chain/get_egs.sh {egs_opts} \
                --cmd "{command}" \
                --cmvn-opts "{cmvn_opts}" \
...
def train_one_iteration(dir, iter, srand, egs_dir,
                        ...
                        backstitch_training_scale=0.0, backstitch_training_interval=1):
    """ Called from subtools/kaldi/steps_multitask/nnet3/chain/train.py for one iteration for

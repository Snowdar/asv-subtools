...
bool CuDevice::SelectGpuIdAuto() {
  ...
  std::vector< std::pair<int, float> > free_mem_free(num_gpus);
  ...
  for(int32 n = 0; n < num_gpus; n++) {
    ...
    switch(ret) {
      case cudaSuccess : {
        ...
        //float mem_free = total > 0 ? free/(float)total : 0;
        float mem_free = total > 0 ? free : 0;
        free_mem_free[n] = std::make_pair(n, mem_free);
  ...
  int32 max_id=0;
  std::sort(free_mem_free.begin(), free_mem_free.end(),
            greater_pair<int, float>);
  // the free_mem_free should be bigger than zero
  KALDI_ASSERT(free_mem_free[max_id].second > 0.0);
  ...
  do {
    ...
    dev_id = free_mem_free[max_id].first;
    mem_free = free_mem_free[max_id].second;

    KALDI_LOG << "Trying to select device: " << dev_id << " (automatically), mem_free: " << mem_free;
    ...
    max_id++;
  } while ((e != cudaSuccess) && (max_id < free_mem_free.size()));

  if (e != cudaSuccess) {
    KALDI_WARN << "Failed to (automatically) select any device";
    return false;
  }
  KALDI_LOG << "Success selecting device " << dev_id << " free mem free: " << mem_free;
  return true;
}

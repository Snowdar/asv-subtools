...
//    if (eg_modified->io[io_ind].name != "output")
//      KALDI_ERR << "Only output can be the name of the egs!" << " but " << eg_modified->io[io_ind].name;
...
/* This function return the type of egs selected.
   0: am egs, and
   1: xvec egs
   select the type of egs according to their proportion
 */
int32 SelectExample(int32 num_am_left, int32 num_xvec_left)
{
  BaseFloat proportion = (BaseFloat)num_am_left / (num_am_left + num_xvec_left);
  int32 i = 1; 
  if (WithProb(proportion))
      i = 0;
  return i;
}
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet3-copy-cvector-egs [options] <am-egs-rspecifier> <xvec-egs-rspecifier> <egs-wspecifier1> [<egs-wspecifier2> ...]\n"
        "\n";
//        "e.g.\n"
...
//        "See also: nnet3-subset-egs, nnet3-get-egs, nnet3-merge-egs, nnet3-shuffle-egs\n";
    ...
    int32 num_am_egs = -1;
    int32 num_xvec_egs = -1;
    BaseFloat am_weight = 1.0, xvec_weight = 1.0;
    std::string am_output_name, xvec_output_name;
    ...
    po.Register("num-am-egs", &num_am_egs, "The number of am egs.");
    po.Register("num-xvec-egs", &num_xvec_egs, "The number of xvec egs.");
    po.Register("am-weight", &am_weight, "The am weight.");
    po.Register("xvec-weight", &xvec_weight, "The xvector weight.");
    po.Register("am-output-name", &am_output_name, "The name of am output node.");
    po.Register("xvec-output-name", &xvec_output_name, "The name of xvector output node.");
    ...
    if (num_am_egs < 0 || num_xvec_egs < 0)
    {
        KALDI_ERR << "num-am-egs and num-xvec-egs must be specified";
    }
    
    if (am_output_name.empty() || xvec_output_name.empty())
        KALDI_ERR << "am-output-name and xvec-output-name must be specified";

    std::string am_examples_rspecifier = po.GetArg(1);
    std::string xvec_examples_rspecifier = po.GetArg(2);

    SequentialNnetExampleReader am_example_reader(am_examples_rspecifier);
    SequentialNnetExampleReader xvec_example_reader(xvec_examples_rspecifier);
    ...
    int32 num_am_left = num_am_egs, num_xvec_left = num_xvec_egs;
    ...
    for (; ;num_read++) {
      ...
      // choose one example (according to their proportion)
      int32 eg_type = SelectExample(num_am_left, num_xvec_left);
      
      std::string key;
      NnetExample eg; 
      BaseFloat weight = 1.0;
      std::string output_name;
      std::string frame_str_tmp;

      // in case one egs is exhausted...
      if (eg_type == 0 && am_example_reader.Done())
          eg_type = 1;
      else if (eg_type == 1 && xvec_example_reader.Done())
          eg_type = 0;

      if (eg_type == 0)
      {
        key = am_example_reader.Key();
        eg = am_example_reader.Value();
        am_example_reader.Next();
        weight = am_weight;
        output_name = am_output_name;
        frame_str_tmp = frame_str;
        num_am_left--;
      }
      else
      {
        key = xvec_example_reader.Key();
        eg = xvec_example_reader.Value();
        xvec_example_reader.Next();
        weight = xvec_weight;
        output_name = xvec_output_name;
        if (frame_str == "" ) {
          frame_str_tmp = frame_str;
        }
        else {
      ...
      for (int32 c = 0; c < count; c++) {
        ...
        if (frame_str == "" && left_context == -1 && right_context == -1 &&
            frame_shift == 0) {
          ScaleAndRenameOutput(weight, output_name, &eg);
          example_writers[index]->Write(key, eg);
          num_written++;
        }
        else{
          NnetExample eg_modified;
          if (SelectFromExample(eg, frame_str_tmp, left_context, right_context,
                                frame_shift, &eg_modified)) {
            ScaleAndRenameOutput(weight, output_name, &eg_modified);
      ...
      // exit only if both readers reach the end
      if (am_example_reader.Done() && xvec_example_reader.Done())
          break;
    }

    if (num_am_left != 0 || num_xvec_left != 0)
      KALDI_ERR << "Unexpected unused example exists.";

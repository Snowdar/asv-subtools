...
// Computes an xvector from a chunk of speech features.
static void RunNnetComputationOffline(const MatrixBase<BaseFloat> &features,
    const Nnet &nnet, NnetComputation &computation,
    Vector<BaseFloat> *xvector) {
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage: nnet3-offline-xvector-compute [options] <raw-nnet-in> "
        "<features-rspecifier> <vector-wspecifier>\n"
        "e.g.: nnet3-offline-xvector-compute --compiled-path-table=compiled.table final.raw scp:feats.scp "
    ...
    std::string use_gpu = "no", path_table="";
    ...
    po.Register("compiled-path-table", &path_table, "A compiled nnet table of path, which used to extract xvectors offline.");
    po.Register("binary", &binary, "If true, read the compiled net with binary mod, which should be consistent with the compiled net file.");
    ...
    typedef unordered_map<int32, NnetComputation> HashType;
    HashType computation;
    if (!path_table.empty()) {
        Timer t;
        SequentialTokenReader table_reader("ark:" + path_table);
        KALDI_LOG << "Compiled nets are exist, so use them to extract xvectors offline.";
        for (; !table_reader.Done();table_reader.Next()) {
            std::ifstream path;
            
            int32 chunk;
            ConvertStringToInteger(table_reader.Key(), &chunk);
            
            if (chunk<min_chunk_size || chunk>max_chunk_size) {
                continue;
            }
            
            if (computation.count(chunk) == 0 && chunk > 0) {
                KALDI_LOG << "Load compiled net for " << chunk;
                std::string str = table_reader.Value();
                path.open(str.c_str());
                computation[chunk].Read(path, binary);
                path.close();
            }
        }
        KALDI_LOG << t.Elapsed() << "s used in reading compiled nets.";
    }
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!pad_input && num_rows < min_chunk_size) {
        ...
        num_fail++;
        continue;
      } else if (num_rows < max_chunk_size) {
          KALDI_LOG << "Chunk size of " << max_chunk_size << " is greater than "
              << "the number of rows in utterance: " << utt
              << ", using chunk size  of " << num_rows;
        this_chunk_size = num_rows;
      } else if (max_chunk_size == -1) {
        this_chunk_size = num_rows;
      }
      ...
      for (int32 chunk_indx = 0; chunk_indx < num_chunks; chunk_indx++) {
        ...
        if (pad_input && offset < min_chunk_size) {
          ...
          if (computation.count(min_chunk_size) == 0) {
              KALDI_LOG << "There is no " << min_chunk_size << " compiled net, so compile it temporarily.";
              RunNnetComputation(padded_features, nnet, &compiler, &xvector);
          }
          else {
              RunNnetComputationOffline(padded_features, nnet, computation[min_chunk_size], &xvector);
          }
        }
        else if (computation.count(offset) == 0) {
            KALDI_LOG << "There is no " << offset << " compiled net, so compile it temporarily.";
            RunNnetComputation(sub_features, nnet, &compiler, &xvector);
        }
        else {
            RunNnetComputationOffline(sub_features, nnet, computation[offset], &xvector);
        }

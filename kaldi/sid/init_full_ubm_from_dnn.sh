...
if [ $# != 4 ]; then
  echo "Usage: subtools/kaldi/steps/init_full_ubm_from_dnn.sh <data-speaker-id> <data-dnn> <dnn-model> <new-ubm-dir>"
  echo "Initializes a full-covariance UBM from DNN posteriors and speaker recognition features."
  echo " e.g.: subtools/kaldi/steps/init_full_ubm_from_dnn.sh data/train data/train_dnn exp/dnn/final.mdl exp/full_ubm"
...
subtools/kaldi/utils/split_data.sh $data_dnn $nj || exit 1;

...
if [ $stage -le 0 ]; then
  ...
  awk '{print $1}' $data/utt2spk | subtools/kaldi/utils/shuffle_list.pl | head -$num_heldout_utts > $temp/valid_uttlist || exit 1;
  # The remaining utterances are used for training egs
  subtools/kaldi/utils/filter_scp.pl --exclude $temp/valid_uttlist $temp/utt2num_frames > $temp/utt2num_frames.train
  subtools/kaldi/utils/filter_scp.pl $temp/valid_uttlist $temp/utt2num_frames > $temp/utt2num_frames.valid
  # Pick a subset of the training list for diagnostics
  awk '{print $1}' $temp/utt2num_frames.train | subtools/kaldi/utils/shuffle_list.pl | head -$num_heldout_utts > $temp/train_subset_uttlist || exit 1;
  subtools/kaldi/utils/filter_scp.pl $temp/train_subset_uttlist <$temp/utt2num_frames.train > $temp/utt2num_frames.train_subset
  # Create a mapping from utterance to speaker ID (an integer)
  awk -v id=0 '{print $1, id++}' $data/spk2utt > $temp/spk2int
  subtools/kaldi/utils/sym2int.pl -f 2 $temp/spk2int $data/utt2spk > $temp/utt2int
  subtools/kaldi/utils/filter_scp.pl $temp/utt2num_frames.train $temp/utt2int > $temp/utt2int.train
  subtools/kaldi/utils/filter_scp.pl $temp/utt2num_frames.valid $temp/utt2int > $temp/utt2int.valid
  subtools/kaldi/utils/filter_scp.pl $temp/utt2num_frames.train_subset $temp/utt2int > $temp/utt2int.train_subset
fi

num_pdfs=$(awk '{print $2}' $temp/utt2int | sort | uniq -c | wc -l)
# The script assumes you've prepared the features ahead of time.
feats="scp,s,cs:subtools/kaldi/utils/filter_scp.pl $temp/ranges.JOB $data/feats.scp |"
train_subset_feats="scp,s,cs:subtools/kaldi/utils/filter_scp.pl $temp/train_subset_ranges.1 $data/feats.scp |"
valid_feats="scp,s,cs:subtools/kaldi/utils/filter_scp.pl $temp/valid_ranges.1 $data/feats.scp |"
...
[ "$num_train_archives" != "" ] && frames_per_iter=$[($num_train_frames*$num_repeats)/($num_train_archives-1)]
[ "$num_train_archives" == "" ] && num_train_archives=$[($num_train_frames*$num_repeats)/$frames_per_iter + 1]
...
if [ $stage -le 1 ]; then
  if [ -e $dir/storage ]; then
    ...
    subtools/kaldi/utils/create_data_link.pl $(for x in $(seq $num_train_archives); do echo $dir/egs.$x.ark; done)
    subtools/kaldi/utils/create_data_link.pl $(for x in $(seq $num_train_archives); do echo $dir/egs_temp.$x.ark; done)
  fi
fi
...
if [ $stage -le 2 ]; then
  ...
  $cmd $dir/log/allocate_examples_train.log \
    subtools/kaldi/sid/nnet3/xvector/allocate_egs.py \

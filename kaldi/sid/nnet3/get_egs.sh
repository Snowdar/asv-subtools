...
cmn=false # for apply-cmvn-sliding
cmn_window=300
center=true
norm_vars=false

vad=true # need $data/vad.scp exist 
#Snowdar 2018-10-01
...
[ "vad" == "true" ] && vad_file=$data/vad.scp # Snowdar 2018-10-01
for f in $data/feats.scp $alidir/ali.1.gz $alidir/final.mdl $alidir/tree $vad_file $extra_files; do
...
cmn_string=""
[ "$cmn" == "true" ] && specifier=ark && cmn_string=" apply-cmvn-sliding --norm-vars=$norm_vars --center=$center --cmn-window=$cmn_window scp:- ark:- |" # Snowdar 2018-10-01

vad_string=""
[ "$vad" == "true" ] && vad_string=" select-voiced-frames $specifier:- scp:$data/vad.scp ark:- |" && specifier=ark # Snowdar 2018-10-01


feats="$specifier,s,cs:subtools/kaldi/utils/filter_scp.pl --exclude $dir/valid_uttlist $sdata/JOB/feats.scp |$cmn_string$vad_string" # there should have no space between | and " if there is no cmd after |
valid_feats="$specifier,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/valid_uttlist $data/feats.scp |$cmn_string$vad_string"
train_subset_feats="$specifier,s,cs:subtools/kaldi/utils/filter_scp.pl $dir/train_subset_uttlist $data/feats.scp |$cmn_string$vad_string"
...
[ "$num_archives" != "" ] && samples_per_iter=$[$num_frames/($num_archives*$frames_per_eg_principal-1)]
[ "$num_archives" == "" ] && num_archives=$[$num_frames/($frames_per_eg_principal*$samples_per_iter)+1]
...
if [ $stage -le 2 ]; then
  ...
  [ "$vad" == "true" ] && cmd_string="select-voiced-ali ark:- scp:$data/vad.scp" # Snowdar 2018-10-01
...
if [ $stage -le 3 ]; then
  ...
  if $generate_egs_scp; then
    ...
    rm -f $dir/{train,valid}_combine.scp
  else
    cat $dir/valid_combine.egs $dir/train_combine.egs > $dir/combine.egs
  fi

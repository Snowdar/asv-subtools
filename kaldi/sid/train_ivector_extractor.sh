...
sdata=$data/split${nj_full}utt;
subtools/kaldi/utils/split_data.sh --per-utt $data $nj_full || exit 1;
...
feats="ark,s,cs:apply-cmvn-sliding --norm-vars=false --center=true --cmn-window=300 scp:$sdata/JOB/feats.scp ark:- | select-voiced-frames ark:- scp,s,cs:$sdata/JOB/vad.scp ark:- |"
fi

...
# ../../subtools/kaldi/steps/nnet2/ for speaker recognition. The main difference is
...
if [ $stage -le -4 ]; then
  ...
  subtools/sid/nnet2/get_lda.sh $lda_opts "${extra_opts[@]}" --left-context $first_left_context --right-context $first_right_context --cmd "$cmd" $data $lang $alidir $dir || exit 1;
fi
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  subtools/sid/nnet2/get_egs2.sh $egs_opts "${extra_opts[@]}" \
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      subtools/kaldi/steps/nnet2/relabel_egs2.sh --cmd "$cmd" --iter $x $dir/ali_$time \

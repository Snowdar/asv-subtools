...
classes="ark:lid/remove_dialect.pl $train_utt2lang \
         | subtools/kaldi/utils/sym2int.pl -f 2 $languages - |"
# A uniform prior.
#subtools/kaldi/utils/sym2int.pl -f 2 $languages \
...
lid/balance_priors_to_test.pl \
    <(lid/remove_dialect.pl <(subtools/kaldi/utils/filter_scp.pl -f 1 \
...
cat $train_dir/posteriors | \
  awk '{max=$3; argmax=3; for(f=3;f<NF;f++) { if ($f>max) 
                          { max=$f; argmax=f; }}  
                          print $1, (argmax - 3); }' | \
  subtools/kaldi/utils/int2sym.pl -f 2 $languages \
    >$train_dir/output

...
# Copyright xmuspeech (Author: Zheng Li 2021-06)
# Apache 2.0
# This is a Launcher for speaker or language - phoneme multi-task learning, based on E-TDNN x-vector.
# Before you start this Launcher, you must ensure that you have already prepared the alginment file for the training set.
...
import libs.egs.egs_multi_task as egs_multi_task #Zheng Li 2021-06-08
import libs.training.optim as optim
import libs.training.lr_scheduler as learn_rate_scheduler
import libs.training.trainer_mt as trainer #Zheng Li 2021-06-08
...
parser.add_argument("--endstage", type=int, default=5,
...
model_params = {
    "extend":True, "SE":False, "se_ratio":4, "training":True, "extracted_embedding":"far","mt_alpha":0.3,
...
subsampling="false" # str, If the chain alignment was uesed, you should turn subsampling = true, Zheng Li 2021-06-08
##--------------------------------------------------##
##
######################################################### START #########################################################
##
...
##
#### Preprocess
if stage <= 3 and endstage >= 0 and utils.is_main_training():
    ...
    kaldi_common.execute_command("sh subtools/pytorch/pipeline/multitask/preprocess_to_egs.sh "
                                 "--subsampling {subsampling} --stage {stage} --endstage {endstage} "
                                 ...
                                 "{traindata} {egs_dir} {ali_dir}".format(subsampling=subsampling,stage=stage, endstage=endstage, nj=preprocess_nj, 
                                 ...
                                 valid_chunk_num_every_utt=valid_chunk_num_every_utt, traindata=traindata, egs_dir=egs_dir, ali_dir=ali_dir))
...
if stage <= 4 <= endstage:
    ...
    bunch, info = egs_multi_task.BaseBunch.get_bunch_from_egsdir(egs_dir, egs_params, loader_params)
    ...
    model = model_py.Xvector(info["feat_dim"], info["num_targets"], info["num_phones"], **model_params) #Zheng Li 2021-06-08
    ...
    trainer = trainer.MultitaskTrainer(package) #Zheng Li 2021-06-08
...
#### Extract xvector
if stage <= 5 <= endstage and utils.is_main_training():
    ...
    to_extracted_data = ["ap19_task_1_train_aug", "task_1"] # All dataset should be in dataroot/prefix.

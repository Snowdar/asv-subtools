...
prefix=fbank_81 # if NULL, datadir will be data/$someset rather than data/$prefix/$someset.

extra_name= # it could be used to mark a trainset in score name

trainset=train  # for convenience with using default config. if NULL, will be set by enrollset

enrollset=eval_enroll  # should be one set only
testset=eval_test  # should be one set only

trials= # if not NULL, will be generated by spk2utt of enrollset and utt2spk of testset in which one speaker is one class

vectordir=
vectortype= # xvector or ivector or any others defined by yourself.If NULL, find it automa_dnntically but only support xvector and ivector
...
clda=128
lda_process="norm-trainlda"
lda_data_config="$trainset"  # if NULL, will be set "$enrollset[$enrollset $testset]"
#lda_data_config="clean"

submean=false
submean_process="lda-getmean" # getmean means computing the global mean vector from a dataset
submean_data_config="$trainset" # if NULL, will be set "$enrollset[$enrollset $testset]"
...
whiten_data_config="$trainset" # if NULL, will be set "$enrollset[$enrollset $testset]"
...
if [ "$eval" == "false" ];then
    ...
    for x in $outsets;do
        echo -e "EER% is "`cat $x`
		subtools/kaldi/sid/compute_min_dcf.py --p-target 0.01 ${outname}.score $trials
    done
else
    echo -e "\n[ Eval Mod ]-[ $testset ]"

...
egs_params = {
    ...
    "aug_params":{"frequency":0.05, "frame":0.2, "rows":2, "cols":1, "random_rows":True,"random_cols":True}
}
...
lr_scheduler_params = {
    ...
    "reduceP.check_interval":2000, # 0 means check metric after every epoch and 1 means every iter. 
...
if stage <= 3 <= endstage:
    ...
    model = model_py.SEResNetXvector(info["feat_dim"], info["num_targets"], **model_params)
...
if stage <= 4 <= endstage and utils.is_main_training():
    ...
    to_extracted_data = ["task2_dev_target","task2_dev_pool"]  # All dataset should be in dataroot/prefix.

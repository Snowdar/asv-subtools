...
vectordir=exp/Trans_Resnet_34_se_am_train_cnceleb_16k_fbank81_bt256/near_epoch_1
vectortype=xvector # xvector or ivector or any others defined by yourself.If NULL, find it automa_dnntically but only support xvector and ivector
...
for the_classfier in $(echo $score | sed 's/-/ /g');do
	...
	et=`date -d  "$endTime" +%s`
	RunTime=$(($et-$st))
	echo "Scoring time is $RunTime s" >> $vectordir/time.txt
